#!/usr/bin/env python3
"""Validate immutable Phase59 reports and export complete reconstruction metrics."""
...
ARMS = {
    "pretraining_balance_control": "16685642",
    "lower_late_pid_pretraining": "16685643",
}
BEAM_METRICS = (
    "source_recall",
    "source_precision",
...
def verify_receipt(receipt):
    ...
    if (
        ...
        or receipt["exit_status"] != 1
        or receipt["sealed_test_accessed"]
    ):
        raise ValueError("Phase59 run did not complete within its authority")


def build(source, recovery):
    prereg = load(
        source / "configs/reconstruction/ht_reconstruction_phase59_20260924.json"
    )
    ...
    payload = {
        "audit_version": "phase59-closeout-v1",
    ...
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase59-20260924/{arm}/{job}"
        receipt_path = (
            source
            / f"artifacts/slurm/reconstruction-phase59/jobs/{job}/attempt-00/receipt.json"
        )
        ...
        assert len(cohort["event_uids"]) == 25
        assert (
...
def main():
    ...
    (args.details_dir / "phase59-all-metrics.json").write_text(
        json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False)
        + "\n"
    )
    with gzip.open(
        args.details_dir / "phase59-all-metrics.csv.gz", "wt", newline=""
    ) as handle:

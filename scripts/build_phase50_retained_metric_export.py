#!/usr/bin/env python3
"""Verify additive Phase50 tree coverage against preserved native reports."""
import argparse
import csv
import gzip
import hashlib
...
ARMS = ('late_adaptation_control', 'late_pid_adaptation')
...
def report_paths(source, arm):
    job = {'late_adaptation_control': '16516116', 'late_pid_adaptation': '16516117'}[arm]
    return sorted((source / f'artifacts/runs/ht-reconstruction-phase50-20260915/{arm}/{job}/full-decay-reports').glob('*.json'))
...
def build(retained):
    output = {'version': 'phase50-retained-tree-export-v1', 'status': 'COMPLETE',
              'original_job_status': 'COMPLETED', 'source_boundary': 'corrected_scientific_audit_source',
              'evaluator_revision': '77b7cefcac3d87ab3c84f31225f362bb18a996d8', 'sealed_test_accessed': False,
...
def main():
    ...
    with gzip.open(args.details_dir / 'phase50-retained-all-metrics.csv.gz', 'wt', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=['arm', 'view', 'metric', 'value'])
        writer.writeheader()
        writer.writerows(payload['metric_rows'])
    tree_export = args.details_dir / 'phase50-retained-tree-metrics.csv.gz'
    tree_count = 0
    ...
    (args.details_dir / 'phase50-retained-all-metrics.json').write_text(json.dumps(payload, sort_keys=True, separators=(',', ':'), allow_nan=False) + '\n')
    ...
    (ROOT / 'docs/_ext/phase50_retained_metric_registry.json').write_text(json.dumps(registry, separators=(',', ':')) + '\n')

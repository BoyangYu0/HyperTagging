"""Matched, parameter-only pretraining refinement before Phase58 reconstruction."""
...
def validate_pretraining_contract(contract, prereg):
    ...
    if contract.get('pretraining_refinement') != expected:
        raise RuntimeError('Phase58 pretraining refinement differs from preregistration')
    c = expected['config']
    if (expected['parent_ranking_weight'] != 2.0
        ...
        or c['lr_schedule_total_steps'] != 2188 or c['seed'] != 20260925
        ...
        raise RuntimeError('Phase58 matched pretraining budget or isolation changed')
...
def run_pretraining_refinement(contract, runtime, cohort, output: Path):
    if output.exists():
        raise RuntimeError('Refusing to overwrite Phase58 pretraining refinement')
    ...
    if (result.steps != 2188 or evidence['step'] != 2188 or not evidence['all_checkpoint_tensors_finite']
        ...
        raise RuntimeError('Phase58 pretraining refinement budget, lineage or cohort failed')
    ...
    if len(training) != 2188 or any(r.get('pretraining_parent_ranking_weight') != config.parent_ranking_weight for r in training):
        raise RuntimeError('Phase58 pretraining history is incomplete')
    if sum(r['batch_events'] for r in training) != 70000 or any(r.get('leaf_pid_training_weight') != config.leaf_pid_phase_weights[(int(r['step'])-1)//547] for r in training):
        raise RuntimeError('Phase58 actual presentations or leaf-PID schedule did not execute')

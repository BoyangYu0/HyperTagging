#!/usr/bin/env python3
"""Export all recorded Phase57 checkpoint and training-log scalar metrics."""
...
from scripts.build_reconstruction_phase57_closeout import ARMS, verify_receipt  # noqa: E402
...
def main():
    ...
    manifest = {'classification': 'RECORDED_TRAINING_WITH_COHORT_DEVIATION',
                'original_job_status': 'FAILED_POST_TRAINING_COHORT_AUDIT', 'sealed_test_accessed': False,
    ...
    with gzip.open(out/'phase57-training-history.csv.gz', 'wt', newline='') as csv_file, gzip.open(out/'phase57-training-history.jsonl.gz', 'wt') as json_file:
        ...
        for arm, job in ARMS.items():
            receipt = source/f'artifacts/slurm/reconstruction-phase57/jobs/{job}/attempt-00/receipt.json'
            verify_receipt(json.loads(receipt.read_text()))
            training = source/f'artifacts/runs/ht-reconstruction-phase57-20260922/{arm}/{job}/training'

...
def main():
    previous = ROOT/'configs/reconstruction/ht_reconstruction_phase48_20260914.json'
    cohort_path = ROOT/'configs/reconstruction/ht_reconstruction_phase49_validation_cohort_20260914.json'
    evidence_path = ROOT/'artifacts/codex/reconstruction_phase48_closeout_20260914.json'
    retained_path = ROOT/'artifacts/codex/reconstruction_phase48_retained_metrics_20260914.json'
    ...
    for key in ('parent_phase47','phase47_closeout_basis','phase47_retained_metric_basis'):
        p.pop(key)
    p.update(study_id='phase49-gradient-safe-pid-adaptation-20260914',
        preregistration_version='hypertagging-reconstruction-phase49-preregistration-v1',
        created_at=datetime.now(timezone.utc).isoformat(),
        scientific_question='With gradient-safe mixed precision in both arms, does unfreezing the transferred detector PID head after step 2188 improve strict topology and primary source-set plus mother-PID recovery versus keeping it frozen, with the same late encoder adaptation?')
    ...
    p['common_training_contract']['seed'] = 20260916
    p['common_training_contract']['balanced_level_replay_contract']['seed'] = 20260916
    ...
    p['parent_phase48']=binding(previous)
    p['phase48_closeout_basis']={**binding(evidence_path),'classification':'completed_corrected_source_no_promotion','selected_next_factor':'late_pid_head_adaptation','sealed_test_accessed':False}
    p['phase48_retained_metric_basis']={**binding(retained_path),'version':retained['version'],'evaluator_revision':retained['evaluator_revision']}
    ...
    p['decision_rules'].update(
        ...
        authority='User authorized Phase48 complete review, publication and exactly one next bounded campaign on 2026-09-14; two 4376-step reconstruction arms, no campaign chain.')
    p['scientific_source_boundary']['limitation']='Gradient-safe autocast fixes suppressed gradients in both arms; train-only statistics and historical encoder81096 match Phase48. Both arms use late encoder adaptation; only PID-head unfreeze timing differs. No physical mother p4-resolution or repaired representability claim.'
    ...
    output=ROOT/'configs/reconstruction/ht_reconstruction_phase49_20260914.json'

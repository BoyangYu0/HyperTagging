#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-47 job contracts."""
...
from scripts.run_reconstruction_phase47 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase47_20260913.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase46_closeout_20260913.json",
    "artifacts/codex/reconstruction_phase46_retained_metrics_20260913.json",
    ...
    "scripts/build_reconstruction_phase47_preregistration.py",
    ...
    "scripts/build_reconstruction_phase47_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase47.py",
    "scripts/run_reconstruction_phase47_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase47_receipt.py",
    "scripts/slurm/render_reconstruction_phase47_campaign.py",
    "scripts/slurm/run_reconstruction_phase47.sbatch",
    "scripts/slurm/submit_reconstruction_phase47_campaign.py",
    ...
    "tests/test_reconstruction_phase47_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase47 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase47 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase47 cohort changed after preregistration")
    ...
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase47 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase47-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase47-{role}-20260913",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase47-20260913/{role}",
        ...
        output = output_dir / f"phase47_{role}_contract_20260913.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase47 contract: {output}")

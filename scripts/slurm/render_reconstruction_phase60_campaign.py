#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-60 job contracts."""
...
from scripts.run_reconstruction_phase60 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
SOURCE_CHECKPOINT = (
    ROOT / "runtime_inputs/reconstruction_phase34_20260904/checkpoint-step-81096.pt"
)
GPU_ENVIRONMENT = "/project/agkuhr/users/boyang/envs/hypertagging-gpu-cu126-v1"
SOURCE_FILES = (
    "scripts/build_reconstruction_phase60_validation_expansion.py",
    "scripts/build_reconstruction_phase60_validation_cohort.py",
    "configs/reconstruction/ht_reconstruction_validation_universe_20260925.json",
    "configs/training_selection/phase60_validation_expansion_20260925/inventory.json",
    "configs/training_selection/phase60_validation_expansion_20260925/roles.json",
    "configs/training_selection/phase60_validation_expansion_20260925/expansion-audit.json",
    ...
    "scripts/run_reconstruction_phase57.py",
    ...
    "artifacts/codex/reconstruction_phase59_closeout_20260925.json",
    "artifacts/codex/reconstruction_phase59_retained_metrics_20260925.json",
    "src/hypertagging/evaluation/retained_tree_checks.py",
    "scripts/evaluate_full_decay.py",
    "scripts/build_reconstruction_phase40_data.py",
    "scripts/build_reconstruction_phase60_preregistration.py",
    "scripts/build_reconstruction_phase60_index_audit.py",
    "artifacts/experiment_readiness/reconstruction_phase60_20260925/fresh-statistics-audit.json",
    ...
    "scripts/run_reconstruction_phase60.py",
    "scripts/run_reconstruction_phase60_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase60_receipt.py",
    "scripts/slurm/render_reconstruction_phase60_campaign.py",
    "scripts/slurm/run_reconstruction_phase60.sbatch",
    "scripts/slurm/submit_reconstruction_phase60_campaign.py",
    "scripts/train_level_reconstruction.py",
    "scripts/run_phase60_pretraining.py",
    "configs/slurm/pretrain_1m_phase3_recovery_20260823.yaml",
    "src/hypertagging/training/pretrain_trainer.py",
    "tests/test_pretraining_parameter_initialization_cpu.py",
    "tests/test_objective_failure_evidence_cpu.py",
    ...
    "tests/test_reconstruction_phase60_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase60 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase60 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase60 cohort changed after preregistration")

    validate_seed_contract(
        prereg["common_training_contract"], json.loads(cohort_path.read_text())["seed"]
    )
    permissions = dict(prereg["authority"])
    test_receipt = args.test_receipt.resolve()
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase60 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase60-test-v1",
    ...
    for arm in prereg["arms"]:
        role = arm["role"]
        source_checkpoint = ROOT / arm["checkpoint"]
        config = {**prereg["common_training_contract"], **arm["overrides"]}
        contract = {
            "contract_version": CONTRACT_VERSION,
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase60-{role}-20260925",
            "arm_role": role,
            ...
            "pretraining_refinement": {
                **prereg["pretraining_refinement"],
                "parent_ranking_weight": 2.0,
                "config": {
                    **prereg["pretraining_refinement"]["config"],
                    "leaf_pid_phase_weights": arm["pretraining_leaf_pid_phase_weights"],
                },
            },
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase60-20260925/{role}",
            "hashed_inputs": hashed_inputs
            + [{"path": arm["checkpoint"], "sha256": sha256(source_checkpoint)}],
        ...
        output = output_dir / f"phase60_{role}_contract_20260925.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase60 contract: {output}")

#!/usr/bin/env python3
"""Atomically submit the two preregistered phase-54 Slurm tasks."""
...
from scripts.run_reconstruction_phase54 import ARM_ROLES, verify_contract  # noqa: E402


WRAPPER = ROOT / "scripts/slurm/run_reconstruction_phase54.sbatch"
...
def submission_command(contract: dict[str, Any], path: Path) -> list[str]:
    ...
    return [
        ...
        f"--comment=phase54:{contract['contract_sha256']}",
...
def main() -> int:
    ...
    if len(args.contract) != 2:
        raise RuntimeError("phase54 requires exactly two contracts")
    output = args.output.resolve()
    if output.exists():
        raise RuntimeError("refusing to overwrite phase54 submission receipt")
    ...
    if tuple(item[1]["arm_role"] for item in verified) != ARM_ROLES:
        raise RuntimeError("phase54 contract arm set changed")
    if len({item[1]["expected_git_sha"] for item in verified}) != 1:
        raise RuntimeError("phase54 contracts do not share one source revision")
    ...
    def payload(status: str) -> dict[str, Any]:
        return {
            "receipt_version": "hypertagging-reconstruction-phase54-submission-v1",
            "created_at": created,
    ...
    try:
        for (path, contract), plan in zip(verified, planned):
            ...
            if not job_id.isdigit():
                raise RuntimeError(f"invalid phase54 Slurm job ID: {raw!r}")
            ...
            if state(record) != "PENDING" or "Reason=JobHeldUser" not in record:
                raise RuntimeError(f"phase54 job {job_id} was not held")
            if f"Comment=phase54:{contract['contract_sha256']}" not in record:
                raise RuntimeError(f"phase54 job {job_id} comment mismatch")
            jobs.append(
                {
                    "arm_role": contract["arm_role"],
                    "task_id": contract["task_id"],
        ...
        for job in jobs:
            ...
            if current not in {"PENDING", "CONFIGURING", "RUNNING"}:
                raise RuntimeError(
                    f"phase54 job {job['job_id']} has unexpected state {current}"
                )

#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-41 job contracts."""
...
from scripts.run_reconstruction_phase41 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase41_20260909.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase40r1_threshold_diagnostic_20260909.json",
    "scripts/evaluate_full_decay.py",
    "scripts/build_reconstruction_phase40_data.py",
    "scripts/build_reconstruction_phase41_preregistration.py",
    "scripts/build_reconstruction_phase41_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase41.py",
    "scripts/run_reconstruction_phase41_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase41_receipt.py",
    "scripts/slurm/render_reconstruction_phase41_campaign.py",
    "scripts/slurm/run_reconstruction_phase41.sbatch",
    "scripts/slurm/submit_reconstruction_phase41_campaign.py",
    ...
    "tests/test_reconstruction_phase41_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase41 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase41 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase41 cohort changed after preregistration")

    permissions = dict(prereg["authority"])
    test_receipt = args.test_receipt.resolve()
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase41 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase41-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase41-{role}-20260909",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase41-20260909/{role}",
        ...
        output = output_dir / f"phase41_{role}_contract_20260909.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase41 contract: {output}")

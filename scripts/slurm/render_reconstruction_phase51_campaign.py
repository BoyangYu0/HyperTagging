...
from scripts.run_reconstruction_phase51 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase51_20260916.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase50_closeout_20260916.json",
    "artifacts/codex/reconstruction_phase50_retained_metrics_20260916.json",
    ...
    "scripts/build_reconstruction_phase51_preregistration.py",
    ...
    "scripts/build_reconstruction_phase51_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase51.py",
    "scripts/run_reconstruction_phase51_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase51_receipt.py",
    "scripts/slurm/render_reconstruction_phase51_campaign.py",
    "scripts/slurm/run_reconstruction_phase51.sbatch",
    "scripts/slurm/submit_reconstruction_phase51_campaign.py",
    ...
    "tests/test_reconstruction_phase51_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase51 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase51 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase51 cohort changed after preregistration")
    ...
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase51 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase51-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase51-{role}-20260916",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase51-20260916/{role}",
        ...
        output = output_dir / f"phase51_{role}_contract_20260916.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase51 contract: {output}")

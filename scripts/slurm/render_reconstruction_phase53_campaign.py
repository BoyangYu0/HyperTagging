#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-53 job contracts."""
...
from scripts.run_reconstruction_phase53 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase53_20260917.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase52_closeout_20260917.json",
    "artifacts/codex/reconstruction_phase52_retained_metrics_20260917.json",
    ...
    "scripts/build_reconstruction_phase53_preregistration.py",
    ...
    "scripts/build_reconstruction_phase53_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase53.py",
    "scripts/run_reconstruction_phase53_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase53_receipt.py",
    "scripts/slurm/render_reconstruction_phase53_campaign.py",
    "scripts/slurm/run_reconstruction_phase53.sbatch",
    "scripts/slurm/submit_reconstruction_phase53_campaign.py",
    ...
    "tests/test_reconstruction_phase53_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase53 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase53 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase53 cohort changed after preregistration")
    ...
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase53 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase53-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase53-{role}-20260917",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase53-20260917/{role}",
        ...
        output = output_dir / f"phase53_{role}_contract_20260917.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase53 contract: {output}")

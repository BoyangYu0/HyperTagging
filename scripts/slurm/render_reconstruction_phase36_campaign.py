#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-36 job contracts."""
...
from scripts.run_reconstruction_phase36 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase36_20260905.json"
...
SOURCE_FILES = (
    ...
    "scripts/build_reconstruction_phase36_preregistration.py",
    "scripts/build_reconstruction_phase36_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase36.py",
    "scripts/run_reconstruction_phase36_full_decay.py",
    "scripts/slurm/finalize_reconstruction_phase36_receipt.py",
    "scripts/slurm/render_reconstruction_phase36_campaign.py",
    "scripts/slurm/run_reconstruction_phase36.sbatch",
    "scripts/slurm/submit_reconstruction_phase36_campaign.py",
    ...
    "tests/test_reconstruction_phase36_campaign_cpu.py",
)
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase36 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase36 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase36 cohort changed after preregistration")

    permissions = dict(prereg["authority"])
    test_receipt = args.test_receipt.resolve()
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase36 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase36-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase36-{role}-20260905",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase36-20260905/{role}",
        ...
        output = output_dir / f"phase36_{role}_contract_20260905.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase36 contract: {output}")

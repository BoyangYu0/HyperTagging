#!/usr/bin/env python3
"""Verify a source-bound, read-only pretraining checkpoint-comparison study."""

from __future__ import annotations

import argparse
import hashlib
import json
import os
from pathlib import Path
import re
import shlex
import subprocess
from typing import Any


ROOT = Path(__file__).resolve().parents[2]
CONTRACT_VERSION = "hypertagging-pretraining-checkpoint-study-v1"
ALLOWED_GRES = {"gpu:h100nvl:1", "gpu:v100:1"}
ALLOWED_STEPS = {2188, 3282, 4376}
HEX64 = re.compile(r"^[0-9a-f]{64}$")
HEX40 = re.compile(r"^[0-9a-f]{40}$")


def _sha256(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


def _inside_root(value: str, *, required_suffix: str | None = None) -> Path:
    path = Path(value)
    candidate = path if path.is_absolute() else ROOT / path
    resolved = candidate.resolve(strict=True)
    try:
        resolved.relative_to(ROOT.resolve())
    except ValueError as error:
        raise RuntimeError("study contract path escapes the repository") from error
    if required_suffix is not None and resolved.suffix != required_suffix:
        raise RuntimeError(f"study contract path must end in {required_suffix}")
    return resolved


def _git_output(*args: str) -> str:
    return subprocess.run(
        ("git", *args), cwd=ROOT, check=True, capture_output=True, text=True, timeout=10
    ).stdout.strip()


def verify_contract_hash(contract: dict[str, Any]) -> None:
    stored = str(contract.get("contract_sha256", ""))
    canonical = {key: value for key, value in contract.items() if key != "contract_sha256"}
    actual = hashlib.sha256(
        json.dumps(canonical, sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
    if stored != actual:
        raise RuntimeError("checkpoint-study contract hash mismatch")


def verify_hashed_inputs(inputs: list[dict[str, str]]) -> None:
    if not inputs:
        raise RuntimeError("checkpoint-study contract has no hashed inputs")
    for item in inputs:
        path = _inside_root(str(item.get("path", "")))
        expected = str(item.get("sha256", ""))
        if not HEX64.fullmatch(expected) or _sha256(path) != expected:
            raise RuntimeError(f"hashed checkpoint-study input changed: {path}")


def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    contract = json.loads(path.read_text(encoding="utf-8"))
    verify_contract_hash(contract)
    verify_hashed_inputs(list(contract.get("hashed_inputs", [])))
    if contract.get("contract_version") != CONTRACT_VERSION:
        raise RuntimeError("unsupported checkpoint-study contract")
    if contract.get("mode") != "read_only_pretraining_checkpoint_comparison":
        raise RuntimeError("checkpoint study is not read-only comparison")
    if contract.get("evaluation_role") != "validation":
        raise RuntimeError("checkpoint study must bind the validation role")
    if contract.get("sealed_test_role_access") != "forbidden":
        raise RuntimeError("checkpoint study must forbid sealed-test access")
    if contract.get("validation_events") != 2000 or contract.get("optimizer_steps") != 0:
        raise RuntimeError("checkpoint study must bind 2,000 events and zero optimizer steps")
    step = int(contract.get("checkpoint_step", -1))
    if step not in ALLOWED_STEPS:
        raise RuntimeError("checkpoint study step is not an approved lineage anchor")
    gres = str(contract.get("gres", ""))
    if gres not in ALLOWED_GRES:
        raise RuntimeError("checkpoint study has unsupported exact GRES")
    checkpoint_sha = str(contract.get("checkpoint_sha256", ""))
    source_sha = str(contract.get("expected_git_sha", ""))
    if not HEX64.fullmatch(checkpoint_sha) or not HEX40.fullmatch(source_sha):
        raise RuntimeError("checkpoint study has an invalid digest")
    checkpoint = _inside_root(str(contract.get("checkpoint", "")), required_suffix=".pt")
    if checkpoint.name != f"checkpoint-step-{step}.pt":
        raise RuntimeError("checkpoint filename and contracted step disagree")
    if _sha256(checkpoint) != checkpoint_sha:
        raise RuntimeError("checkpoint-study checkpoint SHA256 mismatch")
    selection = _inside_root(str(contract.get("selection_manifest", "")), required_suffix=".json")
    dataset_index = _inside_root(str(contract.get("dataset_index", "")), required_suffix=".json")
    output_base = (ROOT / str(contract.get("evaluation_output_base", ""))).resolve()
    expected_parent = (ROOT / "artifacts" / "studies" / "pretraining-checkpoint-comparison").resolve()
    if expected_parent not in output_base.parents:
        raise RuntimeError("checkpoint-study output must remain under its study root")
    gpu_environment = Path(str(contract.get("gpu_environment", "")))
    if not gpu_environment.is_absolute() or not (gpu_environment / "bin/python").is_file():
        raise RuntimeError("checkpoint-study GPU environment is unavailable")
    if _git_output("rev-parse", "HEAD") != source_sha:
        raise RuntimeError("checkpoint-study source Git SHA mismatch")
    tag = str(contract.get("expected_git_tag", ""))
    if not tag or _git_output("rev-list", "-n", "1", tag) != source_sha:
        raise RuntimeError("checkpoint-study source tag mismatch")
    if _git_output("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("tracked worktree changes invalidate checkpoint-study source binding")
    if contract.get("submission_authorized") is not True:
        raise RuntimeError("checkpoint-study contract is not authorized")
    if not os.environ.get("SLURM_JOB_ID"):
        raise RuntimeError("checkpoint study must run inside Slurm")
    if os.environ.get("SLURM_RESTART_COUNT", "0") != "0":
        raise RuntimeError("checkpoint-study jobs may not restart or requeue")
    runtime = {
        "expected_gres": gres,
        "gpu_environment": str(gpu_environment),
        "checkpoint": str(checkpoint),
        "checkpoint_sha256": checkpoint_sha,
        "checkpoint_step": str(step),
        "expected_git_sha": source_sha,
        "selection_manifest": str(selection),
        "dataset_index": str(dataset_index),
        "evaluation_output_base": str(output_base),
    }
    return contract, runtime


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("contract", type=Path)
    parser.add_argument("--shell-output", type=Path, required=True)
    args = parser.parse_args()
    contract, runtime = verify_contract(args.contract.resolve(strict=True))
    args.shell_output.write_text(
        "\n".join(f"{key}={shlex.quote(value)}" for key, value in runtime.items()) + "\n",
        encoding="utf-8",
    )
    print(
        json.dumps(
            {
                "contract_verified": True,
                "contract_sha256": contract["contract_sha256"],
                "checkpoint_step": contract["checkpoint_step"],
            },
            sort_keys=True,
        ),
        flush=True,
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())

#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-42 job contracts."""
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase42_20260909.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase41_closeout_20260909.json",
    "scripts/evaluate_full_decay.py",
    "scripts/build_reconstruction_phase40_data.py",
    "scripts/build_reconstruction_phase42_preregistration.py",
    "scripts/build_reconstruction_phase42_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase42.py",
    "scripts/run_reconstruction_phase42_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase42_receipt.py",
    "scripts/slurm/render_reconstruction_phase42_campaign.py",
    "scripts/slurm/run_reconstruction_phase42.sbatch",
    "scripts/slurm/submit_reconstruction_phase42_campaign.py",
    ...
    "tests/test_reconstruction_phase42_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase42 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase42 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase42 cohort changed after preregistration")

    permissions = dict(prereg["authority"])
    test_receipt = args.test_receipt.resolve()
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase42 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase42-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase42-{role}-20260909",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase42-20260909/{role}",
        ...
        output = output_dir / f"phase42_{role}_contract_20260909.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase42 contract: {output}")

#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-44 job contracts."""
...
from scripts.run_reconstruction_phase44 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase44_20260911.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase43_closeout_20260911.json",
    "scripts/evaluate_full_decay.py",
    "scripts/build_reconstruction_phase40_data.py",
    "scripts/build_reconstruction_phase44_preregistration.py",
    "scripts/build_reconstruction_phase44_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase44.py",
    "scripts/run_reconstruction_phase44_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase44_receipt.py",
    "scripts/slurm/render_reconstruction_phase44_campaign.py",
    "scripts/slurm/run_reconstruction_phase44.sbatch",
    "scripts/slurm/submit_reconstruction_phase44_campaign.py",
    ...
    "tests/test_reconstruction_phase44_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase44 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase44 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase44 cohort changed after preregistration")

    permissions = dict(prereg["authority"])
    test_receipt = args.test_receipt.resolve()
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase44 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase44-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase44-{role}-20260911",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase44-20260911/{role}",
        ...
        output = output_dir / f"phase44_{role}_contract_20260911.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase44 contract: {output}")

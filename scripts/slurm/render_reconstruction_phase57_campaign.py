#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-57 job contracts."""
...
from scripts.run_reconstruction_phase57 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase57_20260922.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase56_closeout_20260922.json",
    "artifacts/codex/reconstruction_phase56_retained_metrics_20260922.json",
    ...
    "scripts/build_reconstruction_phase57_preregistration.py",
    ...
    "scripts/build_reconstruction_phase57_validation_cohort.py",
    ...
    "scripts/run_reconstruction_phase57_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase57_receipt.py",
    "scripts/slurm/render_reconstruction_phase57_campaign.py",
    "scripts/slurm/run_reconstruction_phase57.sbatch",
    "scripts/slurm/submit_reconstruction_phase57_campaign.py",
    "scripts/train_level_reconstruction.py",
    "scripts/run_phase57_pretraining.py",
    ...
    "tests/test_reconstruction_phase57_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase57 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase57 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase57 cohort changed after preregistration")
    ...
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase57 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase57-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase57-{role}-20260922",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase57-20260922/{role}",
        ...
        output = output_dir / f"phase57_{role}_contract_20260922.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase57 contract: {output}")

#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-45 job contracts."""
...
from scripts.run_reconstruction_phase46 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase46_20260912.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase45_closeout_20260912.json",
    "artifacts/codex/reconstruction_phase45_retained_metrics_20260912.json",
    ...
    "scripts/build_reconstruction_phase46_preregistration.py",
    ...
    "scripts/build_reconstruction_phase46_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase46.py",
    "scripts/run_reconstruction_phase46_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase46_receipt.py",
    "scripts/slurm/render_reconstruction_phase46_campaign.py",
    "scripts/slurm/run_reconstruction_phase46.sbatch",
    "scripts/slurm/submit_reconstruction_phase46_campaign.py",
    ...
    "tests/test_reconstruction_phase46_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase46 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase46 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase46 cohort changed after preregistration")
    ...
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase46 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase46-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase46-{role}-20260912",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase46-20260912/{role}",
        ...
        output = output_dir / f"phase46_{role}_contract_20260912.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase46 contract: {output}")

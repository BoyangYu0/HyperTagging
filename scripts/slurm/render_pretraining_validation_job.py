#!/usr/bin/env python3
"""Render, but never submit, an exact read-only pretraining validation job."""
...
from scripts.slurm.verify_pretraining_validation_contract import (  # noqa: E402
    ALLOWED_GRES,
    CONTRACT_VERSION,
)
...
def _repo_path(value: Path) -> Path:
    ...
    try:
        ...
        raise RuntimeError(
            "evaluation input must remain inside the repository"
        ) from error
    return resolved
...
def _live_slurm(gres: str) -> dict[str, Any]:
    ...
    if not any(
        ...
        and not any(
            state in line.lower() for state in ("down", "drain", "fail", "maint")
        )
...
def main() -> int:
    ...
    if args.output.exists():
        raise RuntimeError("refusing to overwrite an existing evaluation contract")
    if _run(("git", "rev-parse", "HEAD")) != args.expected_git_sha:
        raise RuntimeError("expected evaluation source SHA is not checked out")
    if (
        _run(("git", "rev-list", "-n", "1", args.expected_git_tag))
        != args.expected_git_sha
    ):
        raise RuntimeError("expected evaluation tag does not resolve to source SHA")
    ...
    if checkpoint.name != "best.pt":
        raise RuntimeError("scientific evaluation must use best.pt")
    ...
    source_files = (
        ...
        Path("scripts/slurm/evaluate_pretraining_validation.sbatch"),
        Path("scripts/slurm/verify_pretraining_validation_contract.py"),
    ...
    contract: dict[str, Any] = {
        ...
        "checkpoint_selection": {
            "metric": "validation_full_training_objective",
            "mode": "min",
        ...
        "evaluation_output_base": f"artifacts/evaluations/{args.experiment}",
    ...
    command = (
        ...
        "scripts/slurm/evaluate_pretraining_validation.sbatch",
        str(args.output.resolve()),
    )
    print(
        json.dumps({"contract": str(args.output), "command": list(command)}, indent=2)
    )
    return 0

...
from scripts.run_reconstruction_phase45 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase45_20260911.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase44_closeout_20260911.json",
    "artifacts/codex/reconstruction_phase44_retained_metrics_20260911.json",
    ...
    "scripts/build_reconstruction_phase45_preregistration.py",
    "scripts/build_reconstruction_phase45_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase45.py",
    "scripts/run_reconstruction_phase45_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase45_receipt.py",
    "scripts/slurm/render_reconstruction_phase45_campaign.py",
    "scripts/slurm/run_reconstruction_phase45.sbatch",
    "scripts/slurm/submit_reconstruction_phase45_campaign.py",
    ...
    "tests/test_reconstruction_phase45_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase45 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase45 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase45 cohort changed after preregistration")
    ...
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase45 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase45-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase45-{role}-20260911",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase45-20260911/{role}",
        ...
        output = output_dir / f"phase45_{role}_contract_20260911.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase45 contract: {output}")

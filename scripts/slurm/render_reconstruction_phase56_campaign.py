#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-56 job contracts."""
...
from scripts.run_reconstruction_phase56 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase56_20260921.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase55_closeout_20260921.json",
    "artifacts/codex/reconstruction_phase55_retained_metrics_20260921.json",
    ...
    "scripts/build_reconstruction_phase56_preregistration.py",
    ...
    "scripts/build_reconstruction_phase56_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase56.py",
    "scripts/run_reconstruction_phase56_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase56_receipt.py",
    "scripts/slurm/render_reconstruction_phase56_campaign.py",
    "scripts/slurm/run_reconstruction_phase56.sbatch",
    "scripts/slurm/submit_reconstruction_phase56_campaign.py",
    "scripts/train_level_reconstruction.py",
    "scripts/run_phase56_pretraining.py",
    ...
    "tests/test_reconstruction_phase56_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase56 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase56 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase56 cohort changed after preregistration")
    ...
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase56 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase56-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase56-{role}-20260921",
            "arm_role": role,
            ...
            "config": config,
            "pretraining_refinement": {**prereg["pretraining_refinement"], "parent_ranking_weight": arm["pretraining_parent_ranking_weight"]},
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase56-20260921/{role}",
        ...
        output = output_dir / f"phase56_{role}_contract_20260921.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase56 contract: {output}")

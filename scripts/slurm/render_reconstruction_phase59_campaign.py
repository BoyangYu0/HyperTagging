#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-59 job contracts."""
...
from scripts.run_reconstruction_phase59 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase58_closeout_20260924.json",
    "artifacts/codex/reconstruction_phase58_retained_metrics_20260924.json",
    ...
    "scripts/build_reconstruction_phase59_preregistration.py",
    ...
    "scripts/build_reconstruction_phase59_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase59.py",
    "scripts/run_reconstruction_phase59_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase59_receipt.py",
    "scripts/slurm/render_reconstruction_phase59_campaign.py",
    "scripts/slurm/run_reconstruction_phase59.sbatch",
    "scripts/slurm/submit_reconstruction_phase59_campaign.py",
    "scripts/train_level_reconstruction.py",
    "scripts/run_phase59_pretraining.py",
    ...
    "tests/test_reconstruction_phase59_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase59 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase59 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase59 cohort changed after preregistration")
    ...
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase59 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase59-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase59-{role}-20260924",
            "arm_role": role,
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase59-20260924/{role}",
        ...
        output = output_dir / f"phase59_{role}_contract_20260924.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase59 contract: {output}")

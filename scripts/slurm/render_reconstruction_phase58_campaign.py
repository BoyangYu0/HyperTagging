#!/usr/bin/env python3
"""Render the tested, immutable two-arm phase-58 job contracts."""
...
from scripts.run_reconstruction_phase58 import (  # noqa: E402
    CONTRACT_VERSION,
    STUDY_ID,
...
PREREGISTRATION = ROOT / "configs/reconstruction/ht_reconstruction_phase58_20260923.json"
...
SOURCE_FILES = (
    ...
    "artifacts/codex/reconstruction_phase57_closeout_20260923.json",
    "artifacts/codex/reconstruction_phase57_retained_metrics_20260923.json",
    ...
    "scripts/build_reconstruction_phase58_preregistration.py",
    "scripts/build_reconstruction_phase46_index_audit.py",
    "artifacts/experiment_readiness/reconstruction_phase46_20260912/fresh-statistics-audit.json",
    "src/hypertagging/data/heterogeneous.py",
    "src/hypertagging/data/dataset_index.py",
    "scripts/build_reconstruction_phase58_validation_cohort.py",
    "scripts/run_reconstruction_phase35.py",
    "scripts/run_reconstruction_phase35_full_decay.py",
    "scripts/run_reconstruction_phase58.py",
    "scripts/run_reconstruction_phase58_full_decay.py",
    "scripts/run_full_reconstruction_evaluation_suite.py",
    "scripts/slurm/finalize_reconstruction_phase58_receipt.py",
    "scripts/slurm/render_reconstruction_phase58_campaign.py",
    "scripts/slurm/run_reconstruction_phase58.sbatch",
    "scripts/slurm/submit_reconstruction_phase58_campaign.py",
    "scripts/train_level_reconstruction.py",
    "scripts/run_phase58_pretraining.py",
    ...
    "tests/test_reconstruction_phase58_campaign_cpu.py",
...
def main() -> int:
    ...
    if git("rev-list", "-n", "1", args.expected_git_tag) != head:
        raise RuntimeError("phase58 tag does not bind current HEAD")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase58 tracked worktree must be clean")
    ...
    if sha256(cohort_path) != cohort_binding["sha256"]:
        raise RuntimeError("phase58 cohort changed after preregistration")

    validate_seed_contract(prereg["common_training_contract"], json.loads(cohort_path.read_text())["seed"])
    permissions = dict(prereg["authority"])
    test_receipt = args.test_receipt.resolve()
    if test_receipt.exists():
        raise RuntimeError("refusing to overwrite phase58 test receipt")
    test_payload = {
        "receipt_version": "hypertagging-reconstruction-phase58-test-v1",
    ...
    for arm in prereg["arms"]:
        ...
        contract = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "study_id": STUDY_ID,
            "task_id": f"phase58-{role}-20260923",
            "arm_role": role,
            ...
            "config": config,
            "pretraining_refinement": {**prereg["pretraining_refinement"], "parent_ranking_weight": 2.0, "config": {**prereg["pretraining_refinement"]["config"], "leaf_pid_phase_weights": arm["pretraining_leaf_pid_phase_weights"]}},
            ...
            "output_root": f"artifacts/runs/ht-reconstruction-phase58-20260923/{role}",
            "hashed_inputs": hashed_inputs + [{"path": arm["checkpoint"], "sha256": sha256(source_checkpoint)}],
        ...
        output = output_dir / f"phase58_{role}_contract_20260923.json"
        if output.exists():
            raise RuntimeError(f"refusing to overwrite phase58 contract: {output}")

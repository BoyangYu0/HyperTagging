...
CONTRACT_VERSION = "hypertagging-reconstruction-phase45-contract-v1"
STUDY_ID = "phase45-late-encoder-learning-rate-20260911"
COHORT_STUDY_ID = STUDY_ID
ARM_ROLES = (
    "encoder_lr005_control",
    "encoder_lr010",
)
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("phase45 validation exclusion identity changed")
    return tuple(sorted(values))


def validate_closeout_basis(prereg: dict[str, Any]) -> dict[str, Any]:
    closeout = prereg["phase44_closeout_basis"]
    if (prereg.get("study_id") != STUDY_ID
        or closeout.get("classification") != "recovered_diagnostic_study_no_promotion"
        or closeout.get("selected_next_factor") != "late_encoder_lr_multiplier_005_vs_010"
        or closeout.get("sealed_test_accessed") is not False):
        raise RuntimeError("phase45 closeout basis is invalid")
    return closeout
...
def validate_seed_contract(config: dict[str, Any], cohort_seed: int) -> None:
    ...
    if (type(config.get("seed")) is not int
        or config["seed"] != 20260912
        or config["seed"] != cohort_seed
        or config.get("balanced_level_replay_contract", {}).get("seed") != config["seed"]):
        raise RuntimeError("phase45 training, replay, and cohort seeds must agree")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase45 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase45 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase45 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase45 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase45 hashed input changed: {binding['path']}")
    ...
    if (sha256(evidence) != closeout["sha256"]
        ...
        or evidence_record.get("status") != "RECOVERED_DIAGNOSTIC"
        or evidence_record.get("original_job_status") != "FAILED"):
        raise RuntimeError("phase45 closeout evidence changed")
    ...
    if (contract["checkpoint"] != expected_arm["checkpoint"]
        ...
        raise RuntimeError("phase45 preregistered checkpoint or config changed")
    ...
    if set(capacity) != set(ARM_ROLES) or any(
        ...
        raise RuntimeError("phase45 capacity admission is invalid")
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase45-cohort-v1"
        ...
        raise RuntimeError("phase45 untouched cohort contract is invalid")
    ...
    if evaluation_contract.get("retained_tree_checks") != {
        ...
        raise RuntimeError("phase45 requires complete retained-tree and beam checks")
    ...
    if (
        ...
        raise RuntimeError("phase45 scientific selection/gate contract changed")
    if (
        ...
        raise RuntimeError("phase45 shared weighted/recovery contract changed")
    if config.get("max_cardinality_by_level") != [
        [1, 9], [2, 16], [3, 15], [4, 15], [5, 13], [6, 2]
    ]:
        raise RuntimeError("phase45 repaired cardinality contract changed")
    ...
    if (
        ...
        raise RuntimeError("phase45 controlled pretraining-transfer contract changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (ROOT / "artifacts/runs/ht-reconstruction-phase45-20260911").resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase45 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase45 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase45 outputs")
    ...
    if result.steps != 4_376 or sha256(source) != source_before:
        raise RuntimeError("phase45 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase45 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 4_376:
        raise RuntimeError("phase45 final checkpoint is not step 4376")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase45 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase45-training-v1",

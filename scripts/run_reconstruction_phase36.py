#!/usr/bin/env python3
"""Execute one fail-closed phase-36 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase36-contract-v1"
STUDY_ID = "phase36-hierarchy-aware-reconstruction-20260905"
ARM_ROLES = (
    "controlled_baseline",
    "all_level_weighted_recovery_pointer035",
)
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("phase36 validation exclusion identity changed")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase36 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase36 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase36 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase36 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase36 hashed input changed: {binding['path']}")
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase36-cohort-v1"
        ...
        raise RuntimeError("phase36 untouched cohort contract is invalid")
    ...
    if (
        ...
        raise RuntimeError("phase36 scientific selection/gate contract changed")
    if contract["arm_role"] == "controlled_baseline":
        if (
            config.get("level_loss_weights") != []
            or config.get("recovery_objective_weight") != 1.0
            or config.get("rollout_pointer_threshold") != 0.5
            or config.get("unrepresentable_target_policy")
            != "masked_representable_only"
        ):
            raise RuntimeError("phase36 controlled baseline changed")
    else:
        if (
            config.get("level_loss_weights")
            ...
            raise RuntimeError("phase36 weighted/recovery candidate changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (ROOT / "artifacts/runs/ht-reconstruction-phase36-20260905").resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase36 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase36 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase36 outputs")
    ...
    if result.steps != 2_188 or sha256(source) != source_before:
        raise RuntimeError("phase36 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase36 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 2_188:
        raise RuntimeError("phase36 final checkpoint is not step 2188")
    if any(
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase36-training-v1",

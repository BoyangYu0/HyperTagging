#!/usr/bin/env python3
"""Execute one fail-closed phase-58 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase58-contract-v1"
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    if sha256(path) != binding["sha256"]:
        raise RuntimeError("Phase58 validation universe changed")
    ...
    if (
        ...
        raise RuntimeError("Phase58 validation eligibility set is invalid")
    ...
    for key in ("phase57_cohort", "phase57_observed_selection"):
        ...
        path = _repo_file(binding["path"])
        if sha256(path) != binding["sha256"]:
            raise RuntimeError("Phase58 historical cohort binding changed")
    ...
    if (
        ...
        raise RuntimeError(
            "Phase58 requires reused selection set and untouched strict events"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Phase58 every non-selection validation event must be excluded"
        )
...
def preflight_selection(cohort, config, exclusions):
    ...
    if config.get("scientific_mode") is not True:
        raise RuntimeError("Phase58 preflight selector requires actual scientific mode")
    ...
    if tuple(cohort["checkpoint_selection_event_uids"]) != uids or set(uids) & set(
        ...
        raise RuntimeError(
            "Phase58 preflight selector differs from preregistered sequence or touches strict events"
        )
    return {
        "status": "PASS",
...
def validate_closeout_basis(prereg: dict[str, Any]) -> dict[str, Any]:
    closeout = prereg["phase57_closeout_basis"]
    if prereg.get("gradient_execution_contract") != {
        ...
        raise RuntimeError("phase58 requires gradient-safe PID execution")
    if (
        prereg.get("study_id") != STUDY_ID
        or closeout.get("classification")
        != "selection_contaminated_diagnostics_no_promotion"
        or closeout.get("selected_next_factor")
        != "corrected_validation_pretraining_balance_replication"
        or closeout.get("sealed_test_accessed") is not False
    ):
        raise RuntimeError("phase58 closeout basis is invalid")
    return closeout
...
def validate_seed_contract(config: dict[str, Any], cohort_seed: int) -> None:
    ...
    if (
        ...
        or config["seed"] != 20260925
        or config["seed"] != cohort_seed
        or config.get("balanced_level_replay_contract", {}).get("seed")
        != config["seed"]
    ):
        raise RuntimeError("phase58 training, replay, and cohort seeds must agree")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase58 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase58 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase58 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase58 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase58 hashed input changed: {binding['path']}")
    ...
    if (
        ...
        or evidence_record.get("status") != "DIAGNOSTIC_COMPLETE"
    ):
        raise RuntimeError("phase58 closeout evidence changed")
    expected_arm = next(
    ...
    if (
        ...
        raise RuntimeError("phase58 preregistered checkpoint or config changed")
    ...
    if set(capacity) != set(ARM_ROLES) or any(
        ...
        raise RuntimeError("phase58 capacity admission is invalid")

    from scripts.run_phase58_pretraining import validate_pretraining_contract
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase58-cohort-v1"
        ...
        raise RuntimeError(
            "phase58 declared reused-selection/fresh-strict cohort contract is invalid"
        )
    ...
    retained_binding = prereg["phase57_retained_metric_basis"]
    retained_path = _repo_file(retained_binding["path"])
    if sha256(retained_path) != retained_binding["sha256"]:
        raise RuntimeError("phase58 retained-tree evidence hash changed")
    ...
    if (
        ...
        != "selection_contaminated_diagnostic_recovery"
        or retained.get("sealed_test_accessed") is not False
    ):
        raise RuntimeError("phase58 retained-tree evidence is incomplete")
    ...
    if evaluation_contract.get("retained_tree_checks") != {
        ...
        raise RuntimeError("phase58 requires complete retained-tree and beam checks")
    ...
    if (
        ...
        raise RuntimeError("phase58 scientific selection/gate contract changed")
    if (
        ...
        raise RuntimeError("phase58 shared weighted/recovery contract changed")
    if config.get("max_cardinality_by_level") != [
        ...
        raise RuntimeError("phase58 repaired cardinality contract changed")
    ...
    if (
        ...
        raise RuntimeError("phase58 fresh corrected statistics are not bound")
    expected_freeze = {
    ...
    if (
        ...
        raise RuntimeError("phase58 corrected adaptation comparison changed")
    ...
    if (
        ...
        raise RuntimeError("phase58 corrected-source boundary is missing")
    ...
    if (
        ...
        raise RuntimeError("phase58 controlled pretraining-transfer contract changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (
        ROOT / "artifacts/runs/ht-reconstruction-phase58-20260923"
    ).resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase58 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase58 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase58 outputs")
    ...
    from scripts.run_phase58_pretraining import run_pretraining_refinement
    ...
    if result.steps != 4_376 or sha256(source) != source_before:
        raise RuntimeError("phase58 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase58 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 4_376:
        raise RuntimeError("phase58 final checkpoint is not step 4376")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase58 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase58-training-v1",

#!/usr/bin/env python3
"""Build the untouched phase-56 validation-selection and evaluation cohorts."""
...
MANIFEST_VERSION = "hypertagging-reconstruction-phase56-cohort-v1"
STUDY_ID = "phase56-pretraining-parent-replication-20260921"
SELECTION_SEED = 20260923
SELECTION_COUNT = 1_000
...
def main() -> int:
    ...
    if len(train_uids) != 70_000:
        raise RuntimeError("phase56 training role does not contain 70,000 events")
    ...
    phase46_path = ROOT / "configs/reconstruction/ht_reconstruction_phase46_validation_cohort_20260912.json"
    phase46 = json.loads(phase46_path.read_text())
    phase46_uids = set(phase46["checkpoint_selection_event_uids"]) | set(phase46["evaluation_event_uids"])
    ...
    phase47_path = ROOT / "configs/reconstruction/ht_reconstruction_phase47_validation_cohort_20260913.json"
    phase47 = json.loads(phase47_path.read_text())
    phase47_uids = set(phase47["checkpoint_selection_event_uids"]) | set(phase47["evaluation_event_uids"])
    phase48_path = ROOT / "configs/reconstruction/ht_reconstruction_phase48_validation_cohort_20260914.json"
    phase48 = json.loads(phase48_path.read_text())
    phase48_uids = set(phase48['checkpoint_selection_event_uids']) | set(phase48['event_uids'])
    phase49_path = ROOT / "configs/reconstruction/ht_reconstruction_phase49_validation_cohort_20260914.json"
    phase49 = json.loads(phase49_path.read_text())
    phase49_uids = set(phase49['checkpoint_selection_event_uids']) | set(phase49['event_uids'])
    phase50_path = ROOT / "configs/reconstruction/ht_reconstruction_phase50_validation_cohort_20260915.json"
    phase50 = json.loads(phase50_path.read_text())
    phase50_uids = set(phase50["checkpoint_selection_event_uids"]) | set(phase50["event_uids"])
    phase51_path = ROOT / "configs/reconstruction/ht_reconstruction_phase51_validation_cohort_20260916.json"
    phase51 = json.loads(phase51_path.read_text())
    phase51_uids = set(phase51['checkpoint_selection_event_uids']) | set(phase51['event_uids'])
    phase52_path = ROOT / "configs/reconstruction/ht_reconstruction_phase52_validation_cohort_20260917.json"
    phase52 = json.loads(phase52_path.read_text())
    phase52_uids = set(phase52['checkpoint_selection_event_uids']) | set(phase52['event_uids'])
    phase53_path = ROOT / "configs/reconstruction/ht_reconstruction_phase53_validation_cohort_20260917.json"
    phase53 = json.loads(phase53_path.read_text())
    phase53_uids = set(phase53['checkpoint_selection_event_uids']) | set(phase53['event_uids'])
    phase54_path = ROOT / 'configs/reconstruction/ht_reconstruction_phase54_validation_cohort_20260918.json'
    phase54 = json.loads(phase54_path.read_text())
    phase54_uids = set(phase54['checkpoint_selection_event_uids']) | set(phase54['event_uids'])
    phase55_path = ROOT / 'configs/reconstruction/ht_reconstruction_phase55_validation_cohort_20260920.json'
    phase55 = json.loads(phase55_path.read_text())
    phase55_uids = set(phase55['checkpoint_selection_event_uids']) | set(phase55['event_uids'])
    validation_exclusions = phase55_uids | phase54_uids | phase53_uids | phase52_uids | phase51_uids | phase50_uids | phase49_uids | phase48_uids | phase47_uids | phase45_uids | phase46_uids | phase44_uids | set(historical_uids) | phase41_uids | phase43_uids | phase42_uids
    ...
    phase56_selection_set = set(selection_uids)
    phase56_evaluation_set = set(evaluation_uids)
    overlap_audit = {
        "phase55_vs_phase56_selection": len(phase55_uids & phase56_selection_set),
        "phase55_vs_phase56_evaluation": len(phase55_uids & phase56_evaluation_set),
        "phase54_vs_phase56_selection": len(phase54_uids & phase56_selection_set),
        "phase54_vs_phase56_evaluation": len(phase54_uids & phase56_evaluation_set),
        "phase35_train_vs_phase56_selection": len(train_set & phase56_selection_set),
        "phase35_train_vs_phase56_evaluation": len(train_set & phase56_evaluation_set),
        "historical_validation_vs_phase56_selection": len(
            historical_set & phase56_selection_set
        ),
        "historical_validation_vs_phase56_evaluation": len(
            historical_set & phase56_evaluation_set
        ),
        "phase35_selection_vs_phase56_selection": len(
            phase35_selection_set & phase56_selection_set
        ),
        "phase35_selection_vs_phase56_evaluation": len(
            phase35_selection_set & phase56_evaluation_set
        ),
        "phase35_evaluation_vs_phase56_selection": len(
            phase35_evaluation_set & phase56_selection_set
        ),
        "phase35_evaluation_vs_phase56_evaluation": len(
            phase35_evaluation_set & phase56_evaluation_set
        ),
        "phase36_selection_vs_phase56_selection": len(
            phase36_selection_set & phase56_selection_set
        ),
        "phase36_selection_vs_phase56_evaluation": len(
            phase36_selection_set & phase56_evaluation_set
        ),
        "phase36_evaluation_vs_phase56_selection": len(
            phase36_evaluation_set & phase56_selection_set
        ),
        "phase36_evaluation_vs_phase56_evaluation": len(
            phase36_evaluation_set & phase56_evaluation_set
        ),
        "phase37_selection_vs_phase56_selection": len(
            phase37_selection_set & phase56_selection_set
        ),
        "phase37_selection_vs_phase56_evaluation": len(
            phase37_selection_set & phase56_evaluation_set
        ),
        "phase37_evaluation_vs_phase56_selection": len(
            phase37_evaluation_set & phase56_selection_set
        ),
        "phase37_evaluation_vs_phase56_evaluation": len(
            phase37_evaluation_set & phase56_evaluation_set
        ),
        "phase38_selection_vs_phase56_selection": len(
            phase38_selection_set & phase56_selection_set
        ),
        "phase38_selection_vs_phase56_evaluation": len(
            phase38_selection_set & phase56_evaluation_set
        ),
        "phase38_evaluation_vs_phase56_selection": len(
            phase38_evaluation_set & phase56_selection_set
        ),
        "phase38_evaluation_vs_phase56_evaluation": len(
            phase38_evaluation_set & phase56_evaluation_set
        ),
        "phase39_selection_vs_phase56_selection": len(
            phase39_selection_set & phase56_selection_set
        ),
        "phase39_selection_vs_phase56_evaluation": len(
            phase39_selection_set & phase56_evaluation_set
        ),
        "phase39_evaluation_vs_phase56_selection": len(
            phase39_evaluation_set & phase56_selection_set
        ),
        "phase39_evaluation_vs_phase56_evaluation": len(
            phase39_evaluation_set & phase56_evaluation_set
        ),
        "phase40_selection_vs_phase56_selection": len(
            phase40_selection_set & phase56_selection_set
        ),
        "phase40_selection_vs_phase56_evaluation": len(
            phase40_selection_set & phase56_evaluation_set
        ),
        "phase40_evaluation_vs_phase56_selection": len(
            phase40_evaluation_set & phase56_selection_set
        ),
        "phase40_evaluation_vs_phase56_evaluation": len(
            phase40_evaluation_set & phase56_evaluation_set
        ),
        "phase56_selection_vs_phase56_evaluation": len(
            phase56_selection_set & phase56_evaluation_set
        ),
    }
    overlap_audit["phase42_vs_phase56"] = len(phase42_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase43_vs_phase56"] = len(phase43_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase41_vs_phase56"] = len(phase41_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["independent_audit_vs_phase56"] = len(audit_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase46_vs_phase56"] = len(phase46_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase44_vs_phase56"] = len(phase44_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase47_vs_phase56"] = len(phase47_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase48_vs_phase56"] = len(phase48_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase51_vs_phase56"] = len(phase51_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase50_vs_phase56"] = len(phase50_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase49_vs_phase56"] = len(phase49_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase52_vs_phase56"] = len(phase52_uids & (phase56_selection_set | phase56_evaluation_set))
    overlap_audit["phase53_vs_phase56"] = len(phase53_uids & (phase56_selection_set | phase56_evaluation_set))
    if any(overlap_audit.values()):
        raise RuntimeError(f"phase56 cohort is not untouched: {overlap_audit}")
    ...
    payload = {
        ...
        "source_bindings": {
            "phase55_cohort": {"path": str(phase55_path.relative_to(ROOT)), "sha256": sha256(phase55_path)},
            "phase54_cohort": {"path": str(phase54_path.relative_to(ROOT)), "sha256": sha256(phase54_path)},
            "phase52_cohort": {"path": str(phase52_path.relative_to(ROOT)), "sha256": sha256(phase52_path)},
            "phase53_cohort": {"path": str(phase53_path.relative_to(ROOT)), "sha256": sha256(phase53_path)},
            "phase47_cohort": {"path": str(phase47_path.relative_to(ROOT)), "sha256": sha256(phase47_path)},
            "phase48_cohort": {"path": str(phase48_path.relative_to(ROOT)), "sha256": sha256(phase48_path)},
            "phase51_cohort": {"path": str(phase51_path.relative_to(ROOT)), "sha256": sha256(phase51_path)},
            "phase50_cohort": {"path": str(phase50_path.relative_to(ROOT)), "sha256": sha256(phase50_path)},
            "phase49_cohort": {"path": str(phase49_path.relative_to(ROOT)), "sha256": sha256(phase49_path)},
            ...
            "phase46_cohort": {"path": str(phase46_path.relative_to(ROOT)), "sha256": sha256(phase46_path)},

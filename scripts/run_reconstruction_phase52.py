#!/usr/bin/env python3
"""Execute one fail-closed phase-52 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase52-contract-v1"
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("phase52 validation exclusion identity changed")
    return tuple(sorted(values))


def validate_closeout_basis(prereg: dict[str, Any]) -> dict[str, Any]:
    closeout = prereg["phase51_closeout_basis"]
    if prereg.get('gradient_execution_contract') != {'autocast_weight_cache': False, 'trainable_pid_supervision_requires_gradient': True, 'phase48_pid_adaptation_effect_was_not_executed': True}:
        raise RuntimeError('phase52 requires gradient-safe PID execution')
    if (prereg.get("study_id") != STUDY_ID
        ...
        raise RuntimeError("phase52 closeout basis is invalid")
    return closeout
...
def validate_seed_contract(config: dict[str, Any], cohort_seed: int) -> None:
    ...
    if (type(config.get("seed")) is not int
        or config["seed"] != 20260919
        or config["seed"] != cohort_seed
        or config.get("balanced_level_replay_contract", {}).get("seed") != config["seed"]):
        raise RuntimeError("phase52 training, replay, and cohort seeds must agree")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase52 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase52 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase52 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase52 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase52 hashed input changed: {binding['path']}")
    ...
    if (sha256(evidence) != closeout["sha256"]
        ...
        raise RuntimeError("phase52 closeout evidence changed")
    ...
    if (contract["checkpoint"] != expected_arm["checkpoint"]
        ...
        raise RuntimeError("phase52 preregistered checkpoint or config changed")
    ...
    if set(capacity) != set(ARM_ROLES) or any(
        ...
        raise RuntimeError("phase52 capacity admission is invalid")
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase52-cohort-v1"
        ...
        raise RuntimeError("phase52 untouched cohort contract is invalid")
    ...
    retained_binding = prereg["phase51_retained_metric_basis"]
    retained_path = _repo_file(retained_binding["path"])
    if sha256(retained_path) != retained_binding["sha256"]:
        raise RuntimeError("phase52 retained-tree evidence hash changed")
    retained = json.loads(retained_path.read_text())
    if (retained.get("status") != "COMPLETE"
        or retained.get("version") != "phase51-retained-tree-export-v1"
        ...
        raise RuntimeError("phase52 retained-tree evidence is incomplete")
    ...
    if evaluation_contract.get("retained_tree_checks") != {
        ...
        raise RuntimeError("phase52 requires complete retained-tree and beam checks")
    ...
    if (
        ...
        raise RuntimeError("phase52 scientific selection/gate contract changed")
    if (
        config.get("level_loss_weights")
        != [[1, 1.0], [2, 1.0], [3, 1.25], [4, 1.5], [5, 2.0], [6, 3.0]]
        or config.get("recovery_objective_weight") != (4.0 if contract["arm_role"] == "stronger_recovery" else 2.0)
        ...
        raise RuntimeError("phase52 shared weighted/recovery contract changed")
    if config.get("max_cardinality_by_level") != [
        [1, 9], [2, 16], [3, 15], [4, 15], [5, 13], [6, 2]
    ]:
        raise RuntimeError("phase52 repaired cardinality contract changed")
    ...
    if (sha256(statistics_path) != statistics_binding['sha256']
        ...
        raise RuntimeError('phase52 fresh corrected statistics are not bound')
    expected_freeze = {"late_adaptation_control": 4376, "stronger_recovery": 4376}[contract["arm_role"]]
    if (config.get("freeze_pretrained_encoder_steps") != 2188
        ...
        raise RuntimeError("phase52 corrected adaptation comparison changed")
    ...
    if (boundary.get("fresh_train_only_normalization_required") is not True
        ...
        raise RuntimeError("phase52 corrected-source boundary is missing")
    ...
    if (
        ...
        raise RuntimeError("phase52 controlled pretraining-transfer contract changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (ROOT / "artifacts/runs/ht-reconstruction-phase52-20260917").resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase52 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase52 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase52 outputs")
    ...
    if result.steps != 4_376 or sha256(source) != source_before:
        raise RuntimeError("phase52 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase52 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 4_376:
        raise RuntimeError("phase52 final checkpoint is not step 4376")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase52 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase52-training-v1",

#!/usr/bin/env python3
"""Evaluate phase-60 checkpoint tracks and enforce preregistered hierarchy gates."""
...
from scripts.run_reconstruction_phase35_full_decay import (  # noqa: E402
    _artifact,
    _run_evaluator as _legacy_run_evaluator,
...
def _run_evaluator(**kwargs):
    from hypertagging.evaluation.retained_tree_checks import (
        validate_retained_tree_report,
    )

    # Every invocation must explicitly bind its size to the authenticated cohort.
    # The shared legacy runner defaults to 100, while the evaluation contract owns the current size.
    manifest = json.loads(Path(kwargs["cohort_manifest"]).read_text())
    count = kwargs.get(
        "max_events", kwargs["contract"]["evaluation_contract"]["max_events"]
    )
    if count != manifest["event_uid_count"] or count != len(manifest["event_uids"]):
        raise RuntimeError("Phase60 evaluation count differs from bound cohort")
...
def _metric(report: dict[str, Any], *keys: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"phase60 metric is malformed: {'.'.join(keys)}")
    return {
...
def refined_evaluation_runtime(training_result, runtime):
    ...
    if (
        refinement.get("status") != "COMPLETED"
        or refinement.get("step") != 2188
        ...
        raise RuntimeError("Phase60 refined evaluation checkpoint lineage is invalid")
    return {
        **runtime,
...
def verify_evaluation_contract(path, source_root=None):
    ...
    command = [
        sys.executable,
        "-c",
        "import json,pathlib,sys; from scripts.run_reconstruction_phase60 import verify_contract; "
        "print(json.dumps(verify_contract(pathlib.Path(sys.argv[1]))))",
        str(path),
    ]
    result = subprocess.run(
        command, cwd=source_root, check=True, text=True, capture_output=True
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Immutable training checkout for evaluation-only recovery",
    )
    ...
    contract, runtime = verify_evaluation_contract(
        args.contract.resolve(strict=True), args.contract_source_root
    )
    evaluation_contract = dict(contract["evaluation_contract"])
    evaluation_threads = int(evaluation_contract["cpu_threads"])
    deterministic_algorithms = bool(evaluation_contract["deterministic_algorithms"])
    ...
    if (
        ...
        raise RuntimeError("phase60 training result is not evaluation-eligible")
    runtime = refined_evaluation_runtime(training_result, runtime)
    output_dir = args.output_dir.resolve()
    output = args.output.resolve()
    if output.exists() or output_dir.exists():
        raise RuntimeError("refusing to reuse phase60 full-decay outputs")
    ...
    for label, filename in TRACKS.items():
        ...
        if step not in ALLOWED_SELECTION_STEPS:
            raise RuntimeError(f"phase60 {label} selected invalid step {step}")
    ...
    primary_repeat = _run_evaluator(
        ...
        log=repeat_log_path,
        allow_finetuned_encoder=(
            primary_step > int(contract["config"]["freeze_pretrained_encoder_steps"])
        ),
        object_threshold=float(contract["config"]["rollout_object_threshold"]),
        pointer_threshold=float(contract["config"]["rollout_pointer_threshold"]),
        threads=evaluation_threads,
    ...
    if (
        ...
        or beam_report.get("configuration", {})
        .get("beam_search", {})
        .get("truth_used_for_ranking")
        is not False
        or beam_report.get("beam_search", {}).get("evaluated_scopes")
        != ["full", "half"]
    ):
        raise RuntimeError("phase60 beam-search validation contract failed")
    artifacts["primary_complete_target"].update(
        {
    ...
    checks = {
        "nonzero_full_root_completion": endpoints["full_root_completion"]["numerator"]
        ...
        "nonzero_exact_mother_coverage": endpoints["exact_mother_coverage"]["numerator"]
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase60-full-decay-gates-v1",
        "status": "completed",
        ...
        "full_evaluation_bundle": {
            ...
            "contracted_topology_diagnostic": _artifact(contracted_report_path),

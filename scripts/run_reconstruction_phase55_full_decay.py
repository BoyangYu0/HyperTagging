#!/usr/bin/env python3
"""Evaluate phase-55 checkpoint tracks and enforce preregistered hierarchy gates."""
...
def _metric(report: dict[str, Any], *keys: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"phase55 metric is malformed: {'.'.join(keys)}")
    return {
...
def main() -> int:
    ...
    if output.exists() or output_dir.exists():
        raise RuntimeError("refusing to reuse phase55 full-decay outputs")
    ...
    for label, filename in TRACKS.items():
        ...
        if step not in ALLOWED_SELECTION_STEPS:
            raise RuntimeError(f"phase55 {label} selected invalid step {step}")
    ...
    if (
        ...
        raise RuntimeError("phase55 beam-search validation contract failed")
    artifacts["primary_complete_target"].update(
        {
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase55-full-decay-gates-v1",
        "status": "completed",

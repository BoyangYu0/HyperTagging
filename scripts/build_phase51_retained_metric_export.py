#!/usr/bin/env python3
"""Verify additive Phase51 tree coverage against preserved native reports."""
import argparse
import csv
import gzip
import hashlib
...
def report_paths(source, arm):
    job = {'late_adaptation_control': '16523545', 'late_pid_adaptation': '16523546'}[arm]
    return sorted((source / f'artifacts/runs/ht-reconstruction-phase51-20260916/{arm}/{job}/full-decay-reports').glob('*.json'))
...
def build(retained):
    output = {'version': 'phase51-retained-tree-export-v1', 'status': 'COMPLETE',
              'original_job_status': 'COMPLETED', 'source_boundary': 'corrected_scientific_audit_source',
              'evaluator_revision': '637a0f92634cb876fb6cb740e46b46a99aac1ae2', 'sealed_test_accessed': False,
...
def main():
    ...
    with gzip.open(args.details_dir / 'phase51-retained-all-metrics.csv.gz', 'wt', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=['arm', 'view', 'metric', 'value'])
        writer.writeheader()
        writer.writerows(payload['metric_rows'])
    tree_export = args.details_dir / 'phase51-retained-tree-metrics.csv.gz'
    tree_count = 0
    ...
    (args.details_dir / 'phase51-retained-all-metrics.json').write_text(json.dumps(payload, sort_keys=True, separators=(',', ':'), allow_nan=False) + '\n')
    ...
    (ROOT / 'docs/_ext/phase51_retained_metric_registry.json').write_text(json.dumps(registry, separators=(',', ':')) + '\n')

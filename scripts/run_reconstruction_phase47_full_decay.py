#!/usr/bin/env python3
"""Evaluate phase-47 checkpoint tracks and enforce preregistered hierarchy gates."""
...
def _metric(report: dict[str, Any], *keys: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"phase47 metric is malformed: {'.'.join(keys)}")
    return {
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("phase47 training result is not evaluation-eligible")
    output_dir = args.output_dir.resolve()
    output = args.output.resolve()
    if output.exists() or output_dir.exists():
        raise RuntimeError("refusing to reuse phase47 full-decay outputs")
    ...
    for label, filename in TRACKS.items():
        ...
        if step not in ALLOWED_SELECTION_STEPS:
            raise RuntimeError(f"phase47 {label} selected invalid step {step}")
    ...
    if (
        ...
        raise RuntimeError("phase47 beam-search validation contract failed")
    artifacts["primary_complete_target"].update(
        {
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase47-full-decay-gates-v1",
        "status": "completed",

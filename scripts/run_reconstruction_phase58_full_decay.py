#!/usr/bin/env python3
"""Evaluate phase-58 checkpoint tracks and enforce preregistered hierarchy gates."""
...
def _metric(report: dict[str, Any], *keys: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"phase58 metric is malformed: {'.'.join(keys)}")
    return {
...
def refined_evaluation_runtime(training_result, runtime):
    ...
    if (refinement.get("status") != "COMPLETED" or refinement.get("step") != 2188
        ...
        raise RuntimeError("Phase58 refined evaluation checkpoint lineage is invalid")
...
def verify_evaluation_contract(path, source_root=None):
    ...
    command = [sys.executable, "-c",
        "import json,pathlib,sys; from scripts.run_reconstruction_phase58 import verify_contract; "
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("phase58 training result is not evaluation-eligible")
    runtime = refined_evaluation_runtime(training_result, runtime)
    output_dir = args.output_dir.resolve()
    output = args.output.resolve()
    if output.exists() or output_dir.exists():
        raise RuntimeError("refusing to reuse phase58 full-decay outputs")
    ...
    for label, filename in TRACKS.items():
        ...
        if step not in ALLOWED_SELECTION_STEPS:
            raise RuntimeError(f"phase58 {label} selected invalid step {step}")
    ...
    if (
        ...
        raise RuntimeError("phase58 beam-search validation contract failed")
    artifacts["primary_complete_target"].update(
        {
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase58-full-decay-gates-v1",
        "status": "completed",

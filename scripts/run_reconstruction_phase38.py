#!/usr/bin/env python3
"""Execute one fail-closed phase-38 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase38-contract-v1"
STUDY_ID = "phase38-hierarchy-aware-reconstruction-20260906"
ARM_ROLES = (
    "teacher_anchor_050_pointer16_reference",
    "teacher_anchor_050_pointer24",
)
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("phase38 validation exclusion identity changed")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase38 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase38 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase38 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase38 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase38 hashed input changed: {binding['path']}")
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase38-cohort-v1"
        ...
        raise RuntimeError("phase38 untouched cohort contract is invalid")
    validation_exclusions(cohort)

    config = dict(contract["config"])
    gates = dict(contract["post_training_gates"])
    if (
        config.get("max_steps") != 2_188
        or config.get("lr_schedule_total_steps") != 2_188
        ...
        or gates.get("minimum_full_root_completion_numerator") != 1
        or gates.get("minimum_full_lcag_numerator") != 1
        or gates.get("minimum_exact_mother_coverage_numerator") != 1
        or gates.get("minimum_full_source_recall") != 0.15
        or gates.get("minimum_full_source_precision") != 0.70
        or gates.get("minimum_half_source_precision") != 0.40
        or gates.get("minimum_half_perfect_lcag_numerator") != 1
        ...
        raise RuntimeError("phase38 scientific selection/gate contract changed")
    if (
        ...
        raise RuntimeError("phase38 shared weighted/recovery contract changed")
    expected_pointer_weight = (
        16.0
        if contract["arm_role"] == "teacher_anchor_050_pointer16_reference"
        else 24.0
    )
    if config.get("pointer_positive_weight") != expected_pointer_weight:
        raise RuntimeError("phase38 single-factor pointer weight changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (ROOT / "artifacts/runs/ht-reconstruction-phase38-20260906").resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase38 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase38 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase38 outputs")
    ...
    if result.steps != 2_188 or sha256(source) != source_before:
        raise RuntimeError("phase38 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase38 encoder transfer coverage is incomplete")

    checkpoints = {
        name: finite_checkpoint((training_output / filename).resolve(strict=True))
        for name, filename in CHECKPOINT_TRACKS.items()
    }
    if checkpoints["final"]["step"] != 2_188:
        raise RuntimeError("phase38 final checkpoint is not step 2188")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase38 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase38-training-v1",

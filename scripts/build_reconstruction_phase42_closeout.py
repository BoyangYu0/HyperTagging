#!/usr/bin/env python3
"""Validate immutable Phase42 reports and export complete reconstruction metrics."""
...
ARMS = {"pretrain81096_control": "16414762", "pretrain108128": "16414764"}
...
def verify_receipt(receipt):
    ...
    if receipt["status"] != "completed" or receipt["exit_status"] != 0 or receipt["sealed_test_accessed"]:
        raise ValueError("Phase42 run did not complete within its authority")


def build(source):
    prereg = load(source / "configs/reconstruction/ht_reconstruction_phase42_20260909.json")
    ...
    payload = {"audit_version": "phase42-closeout-v1", "status_date": "2026-09-10",
    ...
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase42-20260909/{arm}/{job}"
        receipt_path = source / f"artifacts/slurm/reconstruction-phase42/jobs/{job}/attempt-00/receipt.json"
...
def main():
    ...
    (args.details_dir / "phase42-all-metrics.json").write_text(json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n")
    with gzip.open(args.details_dir / "phase42-all-metrics.csv.gz", "wt", newline="") as handle:

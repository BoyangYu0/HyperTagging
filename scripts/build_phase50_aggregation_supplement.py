...
def build(source):
    payload={'version':'phase50-aggregation-supplement-v1','status':'COMPLETE','source_hashes':[],'metric_rows':[]}
    for arm,job in ARMS.items():
        for path in sorted((source/f'artifacts/runs/ht-reconstruction-phase50-20260915/{arm}/{job}/full-decay-reports').glob('*.json')):
...
def main():
    ...
    (ROOT/'docs/_ext/phase50_aggregation_metric_registry.json').write_text(json.dumps(registry,separators=(',',':'))+'\n');print('PASS',len(registry))

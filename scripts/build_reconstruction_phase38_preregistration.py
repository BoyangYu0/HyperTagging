#!/usr/bin/env python3
"""Freeze the two-arm phase-38 scientific campaign preregistration."""
...
PARENT = ROOT / "configs/reconstruction/ht_reconstruction_phase37_20260906.json"
COHORT = ROOT / "configs/reconstruction/ht_reconstruction_phase38_validation_cohort_20260906.json"
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("phase38 untouched cohort is not ready")
    ...
    common["balanced_level_replay_contract"]["seed"] = 20260907
    payload = {
        "preregistration_version": "hypertagging-reconstruction-phase38-preregistration-v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "study_id": STUDY_ID,
        "scientific_question": (
            "Does increasing the pointer-positive loss weight from 16 to 24, "
            "while holding the phase37 teacher-anchored weighted/recovery contract "
            "fixed, recover exact mothers and roots without exhausting precision?"
        ),
        "parent_phase37": {"path": str(PARENT.relative_to(ROOT)), "sha256": sha256(PARENT)},
        ...
        "arms": [
            {
                "role": "teacher_anchor_050_pointer16_reference",
                "hypothesis": (
                    "Repeating the phase37 teacher-anchor candidate with the established "
                    "pointer-positive weight provides the paired reference."
                ),
                "overrides": {
            ...
            {
                "role": "teacher_anchor_050_pointer24",
                "hypothesis": (
                    "A stronger positive pointer loss recovers missing daughter links "
                    "and enables nonzero exact-mother, LCAG, and root completion while "
                    "the preregistered precision gates limit false associations."
                ),
                "overrides": {
                    "freeze_pretrained_encoder_steps": 1094,
        ...
        "post_training_gates": {
            ...
            "minimum_half_source_precision": 0.40,

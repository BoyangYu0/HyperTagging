#!/usr/bin/env python3
"""Verify additive Phase52 tree coverage against preserved native reports."""
import argparse
import csv
import gzip
import hashlib
...
ARMS = ('late_adaptation_control', 'stronger_recovery')
...
def report_paths(source, arm):
    job = {'late_adaptation_control': '16536720', 'stronger_recovery': '16536721'}[arm]
    return sorted((source / f'artifacts/runs/ht-reconstruction-phase52-20260917/{arm}/{job}/full-decay-reports').glob('*.json'))
...
def build(retained):
    output = {'version': 'phase52-retained-tree-export-v1', 'status': 'COMPLETE',
              'original_job_status': 'COMPLETED', 'source_boundary': 'corrected_scientific_audit_source',
              'evaluator_revision': 'b1bf05fcde0b434dec65f872abf1cdc98db191b6', 'sealed_test_accessed': False,
...
def main():
    ...
    with gzip.open(args.details_dir / 'phase52-retained-all-metrics.csv.gz', 'wt', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=['arm', 'view', 'metric', 'value'])
        writer.writeheader()
        writer.writerows(payload['metric_rows'])
    tree_export = args.details_dir / 'phase52-retained-tree-metrics.csv.gz'
    tree_count = 0
    ...
    (args.details_dir / 'phase52-retained-all-metrics.json').write_text(json.dumps(payload, sort_keys=True, separators=(',', ':'), allow_nan=False) + '\n')
    ...
    (ROOT / 'docs/_ext/phase52_retained_metric_registry.json').write_text(json.dumps(registry, separators=(',', ':')) + '\n')

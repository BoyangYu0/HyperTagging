#!/usr/bin/env python3
"""Execute one fail-closed phase-39 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase39-contract-v1"
STUDY_ID = "phase39-hierarchy-aware-reconstruction-20260907"
ARM_ROLES = (
    "teacher_anchor_050_pointer24_reference",
    "teacher_anchor_050_pointer32",
)
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("phase39 validation exclusion identity changed")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase39 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase39 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase39 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase39 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase39 hashed input changed: {binding['path']}")
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase39-cohort-v1"
        ...
        raise RuntimeError("phase39 untouched cohort contract is invalid")
    ...
    if (
        ...
        raise RuntimeError("phase39 scientific selection/gate contract changed")
    if (
        ...
        raise RuntimeError("phase39 shared weighted/recovery contract changed")
    expected_pointer_weight = (
        24.0
        if contract["arm_role"] == "teacher_anchor_050_pointer24_reference"
        else 32.0
    )
    if config.get("pointer_positive_weight") != expected_pointer_weight:
        raise RuntimeError("phase39 single-factor pointer weight changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (ROOT / "artifacts/runs/ht-reconstruction-phase39-20260907").resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase39 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase39 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase39 outputs")
    ...
    if result.steps != 2_188 or sha256(source) != source_before:
        raise RuntimeError("phase39 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase39 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 2_188:
        raise RuntimeError("phase39 final checkpoint is not step 2188")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase39 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase39-training-v1",

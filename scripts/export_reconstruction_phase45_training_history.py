#!/usr/bin/env python3
"""Export all recorded Phase45 checkpoint and training-log scalar metrics."""
...
from scripts.build_reconstruction_phase45_closeout import ARMS, verify_receipt  # noqa: E402
...
def main():
    ...
    manifest = {'classification': 'PREREGISTERED_PRE_AUDIT_SOURCE',
                'original_job_status': 'COMPLETED', 'sealed_test_accessed': False,
    ...
    with gzip.open(out/'phase45-training-history.csv.gz', 'wt', newline='') as csv_file, gzip.open(out/'phase45-training-history.jsonl.gz', 'wt') as json_file:
        ...
        for arm, job in ARMS.items():
            receipt = source/f'artifacts/slurm/reconstruction-phase45/jobs/{job}/attempt-00/receipt.json'
            verify_receipt(json.loads(receipt.read_text()))
            training = source/f'artifacts/runs/ht-reconstruction-phase45-20260911/{arm}/{job}/training'

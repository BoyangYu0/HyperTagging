#!/usr/bin/env python3
"""Validate immutable Phase46 reports and export complete reconstruction metrics."""
...
ARMS = {"late_adaptation_control": "16467192", "frozen_encoder": "16467193"}
...
def verify_receipt(receipt):
    ...
    if receipt["status"] != "completed" or receipt["exit_status"] != 0 or receipt["sealed_test_accessed"]:
        raise ValueError("Phase46 run did not complete within its authority")


def build(source):
    prereg = load(source / "configs/reconstruction/ht_reconstruction_phase46_20260912.json")
    ...
    payload = {"audit_version": "phase46-closeout-v1", "status_date": "2026-09-13",
    ...
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase46-20260912/{arm}/{job}"
        receipt_path = source / f"artifacts/slurm/reconstruction-phase46/jobs/{job}/attempt-00/receipt.json"
...
def main():
    ...
    (args.details_dir / "phase46-all-metrics.json").write_text(json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n")
    with gzip.open(args.details_dir / "phase46-all-metrics.csv.gz", "wt", newline="") as handle:

#!/usr/bin/env python3
"""Verify additive Phase59 tree coverage against preserved native reports."""
...
ARMS = ("pretraining_balance_control", "lower_late_pid_pretraining")
POINTS = (
    "source_recall",
    "source_precision",
    "lcag_pair_accuracy",
...
def legacy_projection(value):
    if isinstance(value, dict):
        return {
            ...
            if not key.startswith("retained_tree_")
        }
...
def metric_names(value):
    ...
    if isinstance(value, dict):
        ...
        return {
            name: metric_names(item)
            for name, item in zip(names, value.values(), strict=True)
        }
    if isinstance(value, list):
...
def structure_counts(report, scope):
    ...
    return {
        "isolated_leaf_units": sum(
            not row["truth_mother_count"] and len(row["truth_sources"]) == 1
            for row in rows
        ),
        "single_source_composite_units": sum(
            bool(row["truth_mother_count"]) and len(row["truth_sources"]) == 1
            for row in rows
        ),
        "nontrivial_topology_units": sum(
            len(row["truth_sources"]) >= 2 for row in rows
        ),
        "source_empty_units": sum(not row["truth_sources"] for row in rows),
        "events_without_flagged_target_incompatibility": sum(
            bool(event["scopes"][scope]["retained_tree_metrics"]["rows"])
            and all(
                row["target_representable"]
                for row in event["scopes"][scope]["retained_tree_metrics"]["rows"]
            )
            for event in report["events"]
        ),
        "representable_nontrivial_units": sum(
            int(row["target_representable"])
            for row in rows
...
def report_paths(source, arm):
    return sorted((source / arm / "full-decay-reports").glob("*.json"))
        ...
        for path in report_paths(retained, arm):
            ...
            for index, event in enumerate(report["events"]):
                for scope, record in event["scopes"].items():
                    yield from emit(
                        record["retained_tree_metrics"], index, "greedy", -1, scope
                    )
            ...
            event_indices = {
                event["event_uid"]: index
                for index, event in enumerate(report["events"])
            }
            for event in report["beam_search"]["events"]:
                for candidate in event["candidates"]:
                    for scope, result in candidate[
                        "retained_tree_metrics_by_scope"
                    ].items():
                        yield from emit(
                            result,
                            event_indices[event["event_uid"]],
                            "proposal_beam",
...
def build(retained):
    output = {
        "version": "phase59-retained-tree-export-v1",
        "status": "COMPLETE",
        "independent_validation": True,
        "strict_selection_overlap": 0,
        "scientific_mode": "FEASIBILITY_ONLY_NOT_CONFIRMATORY",
        "original_job_status": "FAILED_EVALUATOR_COUNT_MISMATCH",
        "source_boundary": "authenticated_refined_checkpoint_evaluation_recovery",
        "evaluator_revision": "c952365cf9c798886178ebe09402482ad4f23e3a",
        "sealed_test_accessed": False,
        "all_returned_beam_candidates_checked": True,
        "strict_event_count": 25,
        "beam_event_count": 20,
        "source_hashes": [],
        "arms": {},
        "metric_rows": [],
    ...
    for arm in ARMS:
        ...
        for path in report_paths(retained, arm):
            ...
            assert (
                code["git_head"].startswith(output["evaluator_revision"])
                and code["provenance_complete"]
                and not code["worktree_dirty"]
            )
            assert (
                report["device"] == "cpu"
                and report["torch_num_threads"] == 1
                and report["torch_deterministic_algorithms_enabled"]
            )
            assert report["evaluation_role"] == "offline_model_evaluation"
            assert report["context"]["evaluation_split"] == "validation"
            assert set(
                report["context"]["evaluated_uid_split_assignments"].values()
            ) == {"validation"}
            ...
            output["metric_rows"].extend(
                {"arm": arm, "view": path.stem, **row}
                for row in numeric_rows(metric_names(checks))
            )
            output["source_hashes"].extend([digest(path)])
        ...
        def points(summary):
            return {
                **{key: summary[key] for key in POINTS},
                **{
                    key: summary[key]
                    for key in (
                        "unit_count",
                        ...
                        "truth_mother_count",
                        "matched_mother_count",
                        "unit_semantics_counts",
                    )
                },
            }
        ...
        output["arms"][arm] = {
            ...
            "primary_structure": {
                scope: structure_counts(primary, scope) for scope in ("full", "half")
            },
            "primary": {
                scope: points(
                    primary["retained_tree_checks"]["summaries"][f"{scope}/greedy"]
                )
                for scope in ("full", "half")
            },
            "beam": {
                scope: {
                    label.removeprefix(f"{scope}/proposal_beam/"): points(summary)
                    for label, summary in beam["summaries"].items()
                    if label.startswith(f"{scope}/proposal_beam/")
                }
                for scope in ("full", "half")
            },
            "beam_candidate_count": beam["beam"]["full/proposal_beam_normalized_joint"][
                "candidate_count"
            ],
        }
    assert (
        output["arms"][ARMS[0]]["primary_structure"]
        == output["arms"][ARMS[1]]["primary_structure"]
    )
    return output
...
def main():
    ...
    payload = build(args.retained_root)
    args.details_dir.mkdir(parents=True, exist_ok=True)
    with gzip.open(
        args.details_dir / "phase59-retained-all-metrics.csv.gz", "wt", newline=""
    ) as handle:
        writer = csv.DictWriter(handle, fieldnames=["arm", "view", "metric", "value"])
        writer.writeheader()
        writer.writerows(payload["metric_rows"])
    tree_export = args.details_dir / "phase59-retained-tree-metrics.csv.gz"
    tree_count = 0
    ...
    (args.details_dir / "phase59-retained-all-metrics.json").write_text(
    ...
    payload["metric_rows"] = [
        ...
        if not row["metric"].startswith(("by_source_category.", "by_target_shape."))
    ]
    ...
    registry = [
        [row["arm"], row["view"], row["metric"]] for row in payload["metric_rows"]
    ]
    assert len(registry) == len(set(map(tuple, registry)))
    (ROOT / "docs/_ext/phase59_retained_metric_registry.json").write_text(
        json.dumps(registry, separators=(",", ":")) + "\n"
    )
    print(
        json.dumps(
            {
                "aggregate_rows": len(registry),
                "detailed_rows": payload["detailed_metric_count"],

#!/usr/bin/env python3
"""Export all recorded Phase56 checkpoint and training-log scalar metrics."""
...
from scripts.build_reconstruction_phase56_closeout import ARMS, verify_receipt  # noqa: E402
...
def main():
    ...
    manifest = {'classification': 'PREREGISTERED_CORRECTED_SOURCE',
                'comparison_available': False, 'sealed_test_accessed': False,
    ...
    with gzip.open(out/'phase56-training-history.csv.gz', 'wt', newline='') as csv_file, gzip.open(out/'phase56-training-history.jsonl.gz', 'wt') as json_file:
        ...
        for arm, job in ARMS.items():
            receipt = source/f'artifacts/slurm/reconstruction-phase56/jobs/{job}/attempt-00/receipt.json'
            verify_receipt(json.loads(receipt.read_text()))
            training = source/f'artifacts/runs/ht-reconstruction-phase56-20260921/{arm}/{job}/training'
            if arm == 'late_adaptation_control':
                assert not training.exists()
                manifest['arms'][arm] = {'status':'UNAVAILABLE_NOT_TRAINED', 'log_records':0,'log_scalar_rows':0,'checkpoint_scalar_rows':0,'checkpoints':{}}
                continue

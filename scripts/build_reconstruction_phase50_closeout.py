#!/usr/bin/env python3
"""Validate immutable Phase50 reports and export complete reconstruction metrics."""
...
ARMS = {"late_adaptation_control": "16516116", "late_pid_adaptation": "16516117"}
...
def verify_receipt(receipt):
    ...
    if receipt["status"] != "completed" or receipt["exit_status"] != 0 or receipt["sealed_test_accessed"]:
        raise ValueError("Phase50 run did not complete within its authority")


def build(source):
    prereg = load(source / "configs/reconstruction/ht_reconstruction_phase50_20260915.json")
    ...
    payload = {"audit_version": "phase50-closeout-v1", "status_date": "2026-09-16",
    ...
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase50-20260915/{arm}/{job}"
        receipt_path = source / f"artifacts/slurm/reconstruction-phase50/jobs/{job}/attempt-00/receipt.json"
...
def main():
    ...
    (args.details_dir / "phase50-all-metrics.json").write_text(json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n")
    with gzip.open(args.details_dir / "phase50-all-metrics.csv.gz", "wt", newline="") as handle:

#!/usr/bin/env python3
"""Verify additive Phase45 tree coverage against preserved native reports."""
import argparse
import csv
import gzip
import hashlib
...
ARMS = ('encoder_lr005_control', 'encoder_lr010')
...
def report_paths(source, arm):
    job = {'encoder_lr005_control': '16460153', 'encoder_lr010': '16460154'}[arm]
    return sorted((source / f'artifacts/runs/ht-reconstruction-phase45-20260911/{arm}/{job}/full-decay-reports').glob('*.json'))
...
def build(retained):
    output = {'version': 'phase45-retained-tree-export-v1', 'status': 'COMPLETE',
              'original_job_status': 'COMPLETED', 'source_boundary': 'pre_scientific_audit_fixes',
              'evaluator_revision': 'b4b78a1166a77786aaa1b62a3fb6f5fd48ef75ad', 'sealed_test_accessed': False,
...
def main():
    ...
    with gzip.open(args.details_dir / 'phase45-retained-all-metrics.csv.gz', 'wt', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=['arm', 'view', 'metric', 'value'])
        writer.writeheader()
        writer.writerows(payload['metric_rows'])
    tree_export = args.details_dir / 'phase45-retained-tree-metrics.csv.gz'
    tree_count = 0
    ...
    (args.details_dir / 'phase45-retained-all-metrics.json').write_text(json.dumps(payload, sort_keys=True, separators=(',', ':'), allow_nan=False) + '\n')
    ...
    (ROOT / 'docs/_ext/phase45_retained_metric_registry.json').write_text(json.dumps(registry, separators=(',', ':')) + '\n')

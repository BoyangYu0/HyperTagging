#!/usr/bin/env python3
"""Validate immutable Phase56 reports and export complete reconstruction metrics."""
...
ARMS = {"late_adaptation_control": "16623944", "stronger_parent_pretraining": "16623945"}
...
def verify_receipt(receipt):
    ...
    if receipt["status"] != ("failed" if receipt["arm_role"] == "late_adaptation_control" else "completed") or receipt["exit_status"] != (1 if receipt["arm_role"] == "late_adaptation_control" else 0) or receipt["sealed_test_accessed"]:
        raise ValueError("Phase56 run did not complete within its authority")


def build(source, recovery):
    prereg = load(source / "configs/reconstruction/ht_reconstruction_phase56_20260921.json")
    ...
    payload = {"audit_version": "phase56-closeout-v1", "status_date": "2026-09-22",
               "status": "INCOMPLETE_COMPARISON", "metric_completeness": "COMPLETE_AVAILABLE_ARTIFACTS", "evaluation_role": "validation",
               "train_events": 70000, "strict_event_count": 100, "beam_event_count": 20,
               "sealed_test_accessed": False, "promotion_authorized": False,
               "next_study_status": "PREPARED", "source_boundary": "native_phase56_one_arm_pretraining_failure", "comparison_available": False, "arms": {}, "source_hashes": [], "metric_rows": []}
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase56-20260921/{arm}/{job}"
        receipt_path = source / f"artifacts/slurm/reconstruction-phase56/jobs/{job}/attempt-00/receipt.json"
        ...
        for name, binding in receipt["artifacts"].items():
            if binding is not None:
                assert digest(source / binding["path"]) == binding["sha256"]
        if arm == "late_adaptation_control":
            assert all(receipt["artifacts"][k] is None for k in ("full_decay_gates", "pretraining_refinement", "training_result"))
            assert not (run / "training").exists() and not (run / "result.json").exists()
            payload["arms"][arm] = {"status":"FAILED_PRETRAINING", "reconstruction_status":"UNAVAILABLE_NOT_TRAINED", "endpoints":None, "gates":None, "selected":None, "all_gates_passed":None, "primary_repeat_identical":None, "completed_pretraining_steps":1642, "failed_attempted_step":1643, "failure_gradient_ratio":20.1546, "gradient_ratio_limit":20.0}
            payload["source_hashes"].append(digest(receipt_path))
            continue
        result, gates = load(run / "result.json"), load(recovery / arm / "full-decay-gates.json")
        ...
        reports = {}
        report_root = recovery / arm / "full-decay-reports"
        ...
        payload["arms"][arm] = {
            "status": "COMPLETED", "reconstruction_status": "COMPLETE", "selected": gates["selected_checkpoints"]["primary_complete_target"],
        ...
        payload["source_hashes"].extend([digest(receipt_path), digest(run / "result.json"), digest(recovery / arm / "full-decay-gates.json")])
    return payload
...
def main():
    ...
    payload = build(args.source_root, args.recovery_root)
    ...
    (args.details_dir / "phase56-all-metrics.json").write_text(json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n")
    with gzip.open(args.details_dir / "phase56-all-metrics.csv.gz", "wt", newline="") as handle:

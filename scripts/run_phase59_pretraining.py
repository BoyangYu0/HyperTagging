"""Matched, parameter-only pretraining refinement before Phase59 reconstruction."""
...
def validate_pretraining_contract(contract, prereg):
    ...
    if contract.get("pretraining_refinement") != expected:
        raise RuntimeError(
            "Phase59 pretraining refinement differs from preregistration"
        )
    c = expected["config"]
    if (
        expected["parent_ranking_weight"] != 2.0
        or c["leaf_pid_phase_weights"]
        != (
            [1.0, 1.0, 0.1, 0.1]
            if contract["arm_role"] == "lower_late_pid_pretraining"
            else [1.0, 1.0, 0.2, 0.2]
        )
        ...
        raise RuntimeError("Phase59 matched pretraining budget or isolation changed")
...
def run_pretraining_refinement(contract, runtime, cohort, output: Path):
    if output.exists():
        raise RuntimeError("Refusing to overwrite Phase59 pretraining refinement")
    ...
    if (
        ...
        raise RuntimeError(
            "Phase59 pretraining refinement budget, lineage or cohort failed"
        )
    ...
    if len(training) != 2188 or any(
        ...
        raise RuntimeError("Phase59 pretraining history is incomplete")
    if sum(r["batch_events"] for r in training) != 70000 or any(
        ...
        raise RuntimeError(
            "Phase59 actual presentations or leaf-PID schedule did not execute"
        )
    audit = {
        "status": "COMPLETED",

#!/usr/bin/env python3
"""Validate immutable Phase48 reports and export complete reconstruction metrics."""
...
ARMS = {"late_adaptation_control": "16491438", "late_pid_adaptation": "16491439"}
...
def verify_receipt(receipt):
    ...
    if receipt["status"] != "completed" or receipt["exit_status"] != 0 or receipt["sealed_test_accessed"]:
        raise ValueError("Phase48 run did not complete within its authority")


def build(source):
    prereg = load(source / "configs/reconstruction/ht_reconstruction_phase48_20260914.json")
    ...
    payload = {"audit_version": "phase48-closeout-v1", "status_date": "2026-09-14",
               ...
               "next_study_status": "PREPARED", "source_boundary": "corrected_scientific_audit_source", "arms": {}, "source_hashes": [], "metric_rows": []}
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase48-20260914/{arm}/{job}"
        receipt_path = source / f"artifacts/slurm/reconstruction-phase48/jobs/{job}/attempt-00/receipt.json"
...
def main():
    ...
    (args.details_dir / "phase48-all-metrics.json").write_text(json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n")
    with gzip.open(args.details_dir / "phase48-all-metrics.csv.gz", "wt", newline="") as handle:

#!/usr/bin/env python3
"""Validate immutable Phase57 reports and export complete reconstruction metrics."""
...
def verify_receipt(receipt):
    ...
    if receipt["status"] != "failed" or receipt["exit_status"] != 1 or receipt["sealed_test_accessed"]:
        raise ValueError("Phase57 run did not complete within its authority")


def build(source, recovery):
    prereg = load(source / "configs/reconstruction/ht_reconstruction_phase57_20260922.json")
    ...
    payload = {"audit_version": "phase57-closeout-v1", "status_date": "2026-09-23",
               "status": "DIAGNOSTIC_COMPLETE", "independent_validation": False, "strict_selection_overlap": 100, "metric_completeness": "COMPLETE", "evaluation_role": "validation",
               "train_events": 70000, "strict_event_count": 100, "beam_event_count": 20,
               "sealed_test_accessed": False, "promotion_authorized": False,
               "next_study_status": "PREPARED", "source_boundary": "selection_contaminated_diagnostic_recovery", "original_job_status": "FAILED_POST_TRAINING_COHORT_AUDIT", "arms": {}, "source_hashes": [], "metric_rows": []}
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase57-20260922/{arm}/{job}"
        receipt_path = source / f"artifacts/slurm/reconstruction-phase57/jobs/{job}/attempt-00/receipt.json"
        ...
        assert receipt["artifacts"]["full_decay_gates"] is None
        for name, binding in receipt["artifacts"].items():
            if binding is not None:
                assert digest(source / binding["path"]) == binding["sha256"]
        result, gates = load(recovery / arm / "diagnostic-training-evidence.json"), load(recovery / arm / "full-decay-gates.json")
        assert result['status'] == 'TRAINED_COHORT_DEVIATION_DIAGNOSTIC_ONLY'
        assert result['independent_validation'] is False and result['strict_selection_overlap'] == 100
        assert gates['status'] == 'DIAGNOSTIC_COMPLETE' and gates['independent_validation'] is False
        assert gates['all_preregistered_gates_passed'] is False and gates['eligible_for_cross_arm_gate_review'] is False
        ...
        payload["arms"][arm] = {
            ...
            "all_gates_passed": False, "all_numerical_thresholds_passed_diagnostic_only": gates["all_numerical_thresholds_passed_diagnostic_only"], "primary_repeat_identical": True,
        ...
        payload["source_hashes"].extend([digest(receipt_path), digest(recovery / arm / "diagnostic-training-evidence.json"), digest(recovery / arm / "full-decay-gates.json")])
    return payload
...
def main():
    ...
    (args.details_dir / "phase57-all-metrics.json").write_text(json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n")
    with gzip.open(args.details_dir / "phase57-all-metrics.csv.gz", "wt", newline="") as handle:

#!/usr/bin/env python3
"""Verify additive Phase57 tree coverage against preserved native reports."""
import argparse
import csv
import gzip
import hashlib
...
def build(retained):
    output = {'version': 'phase57-retained-tree-export-v1', 'status': 'COMPLETE', 'independent_validation':False, 'strict_selection_overlap':100,
              'original_job_status': 'FAILED_POST_TRAINING_COHORT_AUDIT', 'source_boundary': 'selection_contaminated_diagnostic_recovery',
              'evaluator_revision': 'f52ee4b79e2ade40b9149d63789c90799311f739', 'sealed_test_accessed': False,
              'all_returned_beam_candidates_checked': True,
              'strict_event_count': 100, 'beam_event_count': 20,
              'source_hashes': [], 'arms': {}, 'metric_rows': []}
    for arm in ARMS:
        reports = {}
        for path in report_paths(retained, arm):
            report = json.loads(path.read_text())
            ...
            assert code['git_head'].startswith(output['evaluator_revision']) and code['provenance_complete'] and not code['worktree_dirty']
...
def main():
    ...
    with gzip.open(args.details_dir / 'phase57-retained-all-metrics.csv.gz', 'wt', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=['arm', 'view', 'metric', 'value'])
        writer.writeheader()
        writer.writerows(payload['metric_rows'])
    tree_export = args.details_dir / 'phase57-retained-tree-metrics.csv.gz'
    tree_count = 0
    ...
    (args.details_dir / 'phase57-retained-all-metrics.json').write_text(json.dumps(payload, sort_keys=True, separators=(',', ':'), allow_nan=False) + '\n')
    ...
    (ROOT / 'docs/_ext/phase57_retained_metric_registry.json').write_text(json.dumps(registry, separators=(',', ':')) + '\n')

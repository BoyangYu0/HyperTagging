#!/usr/bin/env python3
"""Verify additive Phase55 tree coverage against preserved native reports."""
import argparse
import csv
import gzip
import hashlib
...
ARMS = ('late_adaptation_control', 'stronger_parent_pretraining')
...
def build(retained):
    output = {'version': 'phase55-retained-tree-export-v1', 'status': 'COMPLETE',
              'original_job_status': 'FAILED_POST_TRAINING_EVALUATION', 'source_boundary': 'authenticated_refined_checkpoint_evaluation_recovery',
              'evaluator_revision': 'e452cb05a6ec4a2bc918f2b2c50cf414e2752f13', 'sealed_test_accessed': False,
...
def main():
    ...
    with gzip.open(args.details_dir / 'phase55-retained-all-metrics.csv.gz', 'wt', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=['arm', 'view', 'metric', 'value'])
        writer.writeheader()
        writer.writerows(payload['metric_rows'])
    tree_export = args.details_dir / 'phase55-retained-tree-metrics.csv.gz'
    tree_count = 0
    ...
    (args.details_dir / 'phase55-retained-all-metrics.json').write_text(json.dumps(payload, sort_keys=True, separators=(',', ':'), allow_nan=False) + '\n')
    ...
    (ROOT / 'docs/_ext/phase55_retained_metric_registry.json').write_text(json.dumps(registry, separators=(',', ':')) + '\n')

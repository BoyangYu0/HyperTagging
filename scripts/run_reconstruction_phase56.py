#!/usr/bin/env python3
"""Execute one fail-closed phase-56 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase56-contract-v1"
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    phase41 = json.loads(_repo_file(bindings["phase41_cohort"]["path"]).read_text())
    phase47 = json.loads(_repo_file(bindings["phase47_cohort"]["path"]).read_text())
    phase50 = json.loads(_repo_file(bindings["phase50_cohort"]["path"]).read_text())
    phase49 = json.loads(_repo_file(bindings["phase49_cohort"]["path"]).read_text())
    phase48 = json.loads(_repo_file(bindings["phase48_cohort"]["path"]).read_text())
    phase46 = json.loads(_repo_file(bindings["phase46_cohort"]["path"]).read_text())
    phase43 = json.loads(_repo_file(bindings["phase43_cohort"]["path"]).read_text())
    phase42 = json.loads(_repo_file(bindings["phase42_cohort"]["path"]).read_text())
    phase44 = json.loads(_repo_file(bindings["phase44_cohort"]["path"]).read_text())
    audit = json.loads(_repo_file(bindings["independent_audit"]["path"]).read_text())
    phase45 = json.loads(_repo_file(bindings["phase45_cohort"]["path"]).read_text())
    phase51 = json.loads(_repo_file(bindings["phase51_cohort"]["path"]).read_text())
    phase52 = json.loads(_repo_file(bindings["phase52_cohort"]["path"]).read_text())
    phase53 = json.loads(_repo_file(bindings["phase53_cohort"]["path"]).read_text())
    phase55 = json.loads(_repo_file(bindings["phase55_cohort"]["path"]).read_text())
    phase54 = json.loads(_repo_file(bindings["phase54_cohort"]["path"]).read_text())
    values = {
        *phase54["checkpoint_selection_event_uids"],
        *phase54["evaluation_event_uids"],
        *phase53["checkpoint_selection_event_uids"],
        *phase53["evaluation_event_uids"],
        *phase55["checkpoint_selection_event_uids"],
        *phase55["evaluation_event_uids"],
        *phase52["checkpoint_selection_event_uids"],
        *phase52["evaluation_event_uids"],
        *phase51["checkpoint_selection_event_uids"],
        *phase51["evaluation_event_uids"],
        *phase49["checkpoint_selection_event_uids"],
        *phase49["evaluation_event_uids"],
        *phase47["checkpoint_selection_event_uids"],
        *phase47["evaluation_event_uids"],
        *phase48["checkpoint_selection_event_uids"],
        *phase48["evaluation_event_uids"],
        *phase50["checkpoint_selection_event_uids"],
        *phase50["evaluation_event_uids"],
        *phase45["checkpoint_selection_event_uids"],
        *phase45["evaluation_event_uids"],
        *audit["event_uids"],
        *phase44["checkpoint_selection_event_uids"],
        *phase44["evaluation_event_uids"],
        *phase43["checkpoint_selection_event_uids"],
        *phase43["evaluation_event_uids"],
        *phase42["checkpoint_selection_event_uids"],
        *phase42["evaluation_event_uids"],
        *phase46["checkpoint_selection_event_uids"],
        *phase46["evaluation_event_uids"],
        *phase41["checkpoint_selection_event_uids"],
        *phase41["evaluation_event_uids"],
    ...
    if (
        ...
        raise RuntimeError("phase56 validation exclusion identity changed")
    return tuple(sorted(values))


def validate_closeout_basis(prereg: dict[str, Any]) -> dict[str, Any]:
    closeout = prereg["phase55_closeout_basis"]
    if prereg.get('gradient_execution_contract') != {'autocast_weight_cache': False, 'trainable_pid_supervision_requires_gradient': True, 'phase48_pid_adaptation_effect_was_not_executed': True}:
        raise RuntimeError('phase56 requires gradient-safe PID execution')
    if (prereg.get("study_id") != STUDY_ID
        or closeout.get("classification") != "recovered_evaluation_corrected_lineage_no_promotion"
        or closeout.get("selected_next_factor") != "pretraining_parent_ranking_weight"
        or closeout.get("sealed_test_accessed") is not False):
        raise RuntimeError("phase56 closeout basis is invalid")
    return closeout
...
def validate_seed_contract(config: dict[str, Any], cohort_seed: int) -> None:
    ...
    if (type(config.get("seed")) is not int
        or config["seed"] != 20260923
        or config["seed"] != cohort_seed
        or config.get("balanced_level_replay_contract", {}).get("seed") != config["seed"]):
        raise RuntimeError("phase56 training, replay, and cohort seeds must agree")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase56 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase56 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase56 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase56 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase56 hashed input changed: {binding['path']}")
    ...
    if (sha256(evidence) != closeout["sha256"]
        or evidence_record.get("metric_completeness") != "COMPLETE"
        or evidence_record.get("status") != "COMPLETED"):
        raise RuntimeError("phase56 closeout evidence changed")
    expected_arm = next(arm for arm in prereg["arms"] if arm["role"] == contract["arm_role"])
    if (contract["checkpoint"] != expected_arm["checkpoint"]
        ...
        or contract["config"] != {**prereg["common_training_contract"], **expected_arm["overrides"]}):
        raise RuntimeError("phase56 preregistered checkpoint or config changed")
    ...
    if set(capacity) != set(ARM_ROLES) or any(
        ...
        raise RuntimeError("phase56 capacity admission is invalid")

    from scripts.run_phase56_pretraining import validate_pretraining_contract
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase56-cohort-v1"
        ...
        raise RuntimeError("phase56 untouched cohort contract is invalid")
    ...
    retained_binding = prereg["phase55_retained_metric_basis"]
    retained_path = _repo_file(retained_binding["path"])
    if sha256(retained_path) != retained_binding["sha256"]:
        raise RuntimeError("phase56 retained-tree evidence hash changed")
    retained = json.loads(retained_path.read_text())
    if (retained.get("status") != "COMPLETE"
        or retained.get("version") != "phase55-retained-tree-export-v1"
        or retained.get("version") != retained_binding.get("version")
        or retained.get("all_returned_beam_candidates_checked") is not True
        or retained.get("original_job_status") != "FAILED_POST_TRAINING_EVALUATION"
        or retained.get("sealed_test_accessed") is not False):
        raise RuntimeError("phase56 retained-tree evidence is incomplete")
    evaluation_contract = dict(contract["evaluation_contract"])
    if evaluation_contract.get("retained_tree_checks") != {
        "version": "retained-direct-tree-checks-v1", "required": True,
        "scopes": ["full", "half"], "all_returned_beam_candidates": True,
        "population": "all_explicit_retained_roots_including_outside_training_policy",
        "replaces_original_gates": False,
    }:
        raise RuntimeError("phase56 requires complete retained-tree and beam checks")
    ...
    if (
        ...
        raise RuntimeError("phase56 scientific selection/gate contract changed")
    if (
        ...
        raise RuntimeError("phase56 shared weighted/recovery contract changed")
    if config.get("max_cardinality_by_level") != [
        [1, 9], [2, 16], [3, 15], [4, 15], [5, 13], [6, 2]
    ]:
        raise RuntimeError("phase56 repaired cardinality contract changed")
    ...
    if (sha256(statistics_path) != statistics_binding['sha256']
        ...
        or statistics.get('index_sha256') != sha256(_repo_file(contract['data']['dataset_index']))
        or any(sha256(_repo_file(path)) != digest for path, digest in statistics['source_files'].items())):
        raise RuntimeError('phase56 fresh corrected statistics are not bound')
    expected_freeze = {"late_adaptation_control": 4376, "stronger_parent_pretraining": 4376}[contract["arm_role"]]
    if (config.get("freeze_pretrained_encoder_steps") != 2188
        ...
        or config.get("resume") or config.get("resume_from")):
        raise RuntimeError("phase56 corrected adaptation comparison changed")
    boundary = prereg.get("scientific_source_boundary", {})
    if (boundary.get("fresh_train_only_normalization_required") is not True
        or boundary.get("both_arms_use_corrected_training_and_inference") is not True
        or boundary.get("resume_forbidden") is not True):
        raise RuntimeError("phase56 corrected-source boundary is missing")
    ...
    if (
        ...
        raise RuntimeError("phase56 controlled pretraining-transfer contract changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (ROOT / "artifacts/runs/ht-reconstruction-phase56-20260921").resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase56 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase56 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase56 outputs")
    ...
    from scripts.run_phase56_pretraining import run_pretraining_refinement
    refinement = run_pretraining_refinement(contract, runtime, cohort, training_output.parent / "pretraining")
    runtime = {**runtime, "checkpoint": refinement["checkpoint"], "checkpoint_sha256": refinement["checkpoint_sha256"], "checkpoint_step": "2188"}
    ...
    if result.steps != 4_376 or sha256(source) != source_before:
        raise RuntimeError("phase56 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase56 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 4_376:
        raise RuntimeError("phase56 final checkpoint is not step 4376")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase56 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase56-training-v1",

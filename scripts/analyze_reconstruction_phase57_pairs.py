#!/usr/bin/env python3
"""Paired event-cluster uncertainty and exact-component depth audit for Phase57."""
import argparse
...
from scripts.build_reconstruction_phase57_closeout import ARMS
...
def main():
    ...
    reports=[json.loads((args.source_root/f'{arm}/full-decay-reports/primary_complete_target_direct.json').read_text()) for arm,job in ARMS.items()]
    ...
    output={'version':'phase57-paired-event-bootstrap-v1','replicates':10000,'seed':20260922,'interval':'paired event-cluster percentile 95%, ratio of summed counts','limitation':'In-sample diagnostic only: all100 evaluation events were used for checkpoint selection. Conditional on two trained models; does not measure training-seed uncertainty. Degenerate all-zero intervals do not imply zero population uncertainty.','scopes':{},'perfect_components':{}}

"""Matched, parameter-only pretraining refinement before Phase57 reconstruction."""
...
def validate_pretraining_contract(contract, prereg):
    ...
    expected['config'] = {**expected['config'], 'leaf_pid_phase_weights': arm['pretraining_leaf_pid_phase_weights']}
    if contract.get('pretraining_refinement') != expected:
        raise RuntimeError('Phase57 pretraining refinement differs from preregistration')
    c = expected['config']
    if (expected['parent_ranking_weight'] != 2.0
        or c['leaf_pid_phase_weights'] != ([1.0, 1.0, 0.2, 0.2] if contract['arm_role'] == 'lower_late_pid_pretraining' else [1.0, 1.0, 0.4, 0.4])
        or c['objective_dominance_ratio'] != 20.0
        or c['max_steps'] != 2188 or c['batch_size'] != 32
        or c['lr_schedule_total_steps'] != 2188 or c['seed'] != 20260924
        ...
        raise RuntimeError('Phase57 matched pretraining budget or isolation changed')
...
def run_pretraining_refinement(contract, runtime, cohort, output: Path):
    if output.exists():
        raise RuntimeError('Refusing to overwrite Phase57 pretraining refinement')
    ...
    if (result.steps != 2188 or evidence['step'] != 2188 or not evidence['all_checkpoint_tensors_finite']
        ...
        or tuple(payload['config']['leaf_pid_phase_weights']) != config.leaf_pid_phase_weights):
        raise RuntimeError('Phase57 pretraining refinement budget, lineage or cohort failed')
    ...
    if len(training) != 2188 or any(r.get('pretraining_parent_ranking_weight') != config.parent_ranking_weight for r in training):
        raise RuntimeError('Phase57 pretraining history is incomplete')
    if sum(r['batch_events'] for r in training) != 70000 or any(r.get('leaf_pid_training_weight') != config.leaf_pid_phase_weights[(int(r['step'])-1)//547] for r in training):
        raise RuntimeError('Phase57 actual presentations or leaf-PID schedule did not execute')

#!/usr/bin/env python3
"""Execute one fail-closed phase-40r1 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase40r1-contract-v1"
STUDY_ID = "phase40r1-capacity-repair-20260908"
COHORT_STUDY_ID = "phase40-hierarchy-aware-reconstruction-20260907"
ARM_ROLES = (
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("phase40r1 validation exclusion identity changed")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase40r1 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase40r1 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase40r1 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase40r1 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase40r1 hashed input changed: {binding['path']}")

    prereg_path = _repo_file(contract["preregistration"]["path"])
    prereg = json.loads(prereg_path.read_text(encoding="utf-8"))
    replacement = dict(prereg.get("replacement_basis", {}))
    if (
        prereg.get("study_id") != STUDY_ID
        or replacement.get("classification") != "admission_only_capacity_repair"
        or replacement.get("phase40_optimizer_steps") != 0
        or replacement.get("phase40_checkpoint_selection_cohort_evaluated") is not False
        or replacement.get("phase40_sealed_test_accessed") is not False
        or replacement.get("only_scientific_contract_change")
        != "level_4_max_cardinality_14_to_15"
    ):
        raise RuntimeError("phase40r1 replacement basis is invalid")
    for evidence in replacement.get("failure_evidence", []):
        receipt = _repo_file(evidence["receipt"])
        error_log = _repo_file(evidence["error_log"])
        if (
            sha256(receipt) != evidence["receipt_sha256"]
            or sha256(error_log) != evidence["error_log_sha256"]
            or evidence.get("optimizer_steps") != 0
            or evidence.get("checkpoint_selection_cohort_evaluated") is not False
            or evidence.get("sealed_test_accessed") is not False
        ):
            raise RuntimeError("phase40r1 failure evidence changed")
    ...
    if set(capacity) != set(ARM_ROLES) or any(
        ...
        raise RuntimeError("phase40r1 capacity admission is invalid")
    ...
    if (
        ...
        raise RuntimeError("phase40r1 untouched cohort contract is invalid")
    ...
    if (
        ...
        raise RuntimeError("phase40r1 scientific selection/gate contract changed")
    if (
        ...
        raise RuntimeError("phase40r1 shared weighted/recovery contract changed")
    if config.get("max_cardinality_by_level") != [
        [1, 9], [2, 16], [3, 15], [4, 15], [5, 13], [6, 2]
    ]:
        raise RuntimeError("phase40r1 repaired cardinality contract changed")
    expected_queries = (
    ...
    if (
        ...
        raise RuntimeError("phase40r1 controlled data/query scale contract changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (ROOT / "artifacts/runs/ht-reconstruction-phase40r1-20260908").resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase40r1 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase40r1 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase40r1 outputs")
    ...
    if result.steps != 4_376 or sha256(source) != source_before:
        raise RuntimeError("phase40r1 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase40r1 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 4_376:
        raise RuntimeError("phase40r1 final checkpoint is not step 4376")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase40r1 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase40r1-training-v1",

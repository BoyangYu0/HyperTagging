"""Matched, parameter-only pretraining refinement before Phase60 reconstruction."""
...
def validate_pretraining_contract(contract, prereg):
    ...
    if contract.get("pretraining_refinement") != expected:
        raise RuntimeError(
            "Phase60 pretraining refinement differs from preregistration"
        )
    c = expected["config"]
    if (
        expected["parent_ranking_weight"] != 2.0
        ...
        raise RuntimeError("Phase60 matched pretraining budget or isolation changed")
...
def run_pretraining_refinement(contract, runtime, cohort, output: Path):
    if output.exists():
        raise RuntimeError("Refusing to overwrite Phase60 pretraining refinement")
    ...
    if (
        ...
        raise RuntimeError(
            "Phase60 pretraining refinement budget, lineage or cohort failed"
        )
    ...
    if len(training) != 2188 or any(
        ...
        raise RuntimeError("Phase60 pretraining history is incomplete")
    if sum(r["batch_events"] for r in training) != 70000 or any(
        ...
        raise RuntimeError(
            "Phase60 actual presentations or leaf-PID schedule did not execute"
        )
    audit = {
        "status": "COMPLETED",

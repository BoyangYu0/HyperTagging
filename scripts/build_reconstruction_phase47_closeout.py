#!/usr/bin/env python3
"""Validate immutable Phase47 reports and export complete reconstruction metrics."""
...
ARMS = {"late_adaptation_control": "16470329", "frozen_encoder": "16470330"}
...
def verify_receipt(receipt):
    ...
    if receipt["status"] != "completed" or receipt["exit_status"] != 0 or receipt["sealed_test_accessed"]:
        raise ValueError("Phase47 run did not complete within its authority")


def build(source):
    prereg = load(source / "configs/reconstruction/ht_reconstruction_phase47_20260913.json")
    ...
    payload = {"audit_version": "phase47-closeout-v1", "status_date": "2026-09-14",
    ...
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase47-20260913/{arm}/{job}"
        receipt_path = source / f"artifacts/slurm/reconstruction-phase47/jobs/{job}/attempt-00/receipt.json"
...
def main():
    ...
    (args.details_dir / "phase47-all-metrics.json").write_text(json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n")
    with gzip.open(args.details_dir / "phase47-all-metrics.csv.gz", "wt", newline="") as handle:

...
def _metric(report: dict[str, Any], *keys: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"phase45 metric is malformed: {'.'.join(keys)}")
    return {
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("phase45 training result is not evaluation-eligible")
    output_dir = args.output_dir.resolve()
    output = args.output.resolve()
    if output.exists() or output_dir.exists():
        raise RuntimeError("refusing to reuse phase45 full-decay outputs")
    ...
    for label, filename in TRACKS.items():
        ...
        if step not in ALLOWED_SELECTION_STEPS:
            raise RuntimeError(f"phase45 {label} selected invalid step {step}")
    ...
    if (
        ...
        raise RuntimeError("phase45 beam-search validation contract failed")
    artifacts["primary_complete_target"].update(
        {
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase45-full-decay-gates-v1",
        "status": "completed",

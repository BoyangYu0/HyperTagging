#!/usr/bin/env python3
"""Evaluate phase-39 checkpoint tracks and enforce preregistered hierarchy gates."""
...
def _metric(report: dict[str, Any], *keys: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"phase39 metric is malformed: {'.'.join(keys)}")
    return {
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("phase39 training result is not evaluation-eligible")
    output_dir = args.output_dir.resolve()
    output = args.output.resolve()
    if output.exists() or output_dir.exists():
        raise RuntimeError("refusing to reuse phase39 full-decay outputs")
    ...
    for label, filename in TRACKS.items():
        ...
        if step not in ALLOWED_SELECTION_STEPS:
            raise RuntimeError(f"phase39 {label} selected invalid step {step}")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase39-full-decay-gates-v1",
        "status": "completed",

#!/usr/bin/env python3
"""Verify additive Phase58 tree coverage against preserved native reports."""
...
ARMS = ("lower_late_pid_pretraining",)
POINTS = (
    "source_recall",
    "source_precision",
    "lcag_pair_accuracy",
...
def build(retained):
    output = {
        "version": "phase58-retained-tree-export-v1",
        "status": "COMPLETE",
        "unavailable_arms": {"pretraining_balance_control": "UNAVAILABLE_NOT_TRAINED"},
        "source_boundary": "native_phase58_one_arm_pretraining_failure",
        "evaluator_revision": "64ba0674430ff20fa3d1f827e9677d1184ce6041",
...
def main():
    ...
    with gzip.open(
        args.details_dir / "phase58-retained-all-metrics.csv.gz", "wt", newline=""
    ) as handle:
        writer = csv.DictWriter(handle, fieldnames=["arm", "view", "metric", "value"])
        writer.writeheader()
        writer.writerows(payload["metric_rows"])
    tree_export = args.details_dir / "phase58-retained-tree-metrics.csv.gz"
    tree_count = 0
    ...
    (args.details_dir / "phase58-retained-all-metrics.json").write_text(
    ...
    (ROOT / "docs/_ext/phase58_retained_metric_registry.json").write_text(

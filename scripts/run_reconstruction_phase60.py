#!/usr/bin/env python3
"""Execute one fail-closed phase-60 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase60-contract-v1"
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    if sha256(path) != binding["sha256"]:
        raise RuntimeError("Phase60 validation universe changed")
    ...
    if (
        ...
        raise RuntimeError("Phase60 validation eligibility set is invalid")
    ...
    if sha256(old_path) != binding["sha256"]:
        raise RuntimeError("Phase60 historical validation universe changed")
    historical = set(json.loads(old_path.read_text())["event_uids"])
    if (
        len(historical) != 50000
        ...
        raise RuntimeError(
            "Phase60 requires fresh selection and strict events from added sources"
        )
    values = all_uids - set(selected)
    identity = excluded_event_uids_contract(tuple(values))
    if (
        identity["excluded_event_uid_count"] != 99000
        ...
        raise RuntimeError(
            "Phase60 every non-selection validation event must be excluded"
        )
...
def validate_evaluation_population(evaluation, cohort):
    ...
    if (
        ...
        raise RuntimeError("Phase60 evaluator population differs from its bound cohort")
...
def preflight_selection(cohort, config, exclusions):
    ...
    if config.get("scientific_mode") is not True:
        raise RuntimeError("Phase60 preflight selector requires actual scientific mode")
    ...
    if tuple(cohort["checkpoint_selection_event_uids"]) != uids or set(uids) & set(
        ...
        raise RuntimeError(
            "Phase60 preflight selector differs from preregistered sequence or touches strict events"
        )
    return {
        "status": "PASS",
...
def validate_closeout_basis(prereg: dict[str, Any]) -> dict[str, Any]:
    if (
        prereg.get("pilot_classification") != "EXPLORATORY_FRESH_VALIDATION_REPLICATION"
        or prereg.get("validation_budget", {}).get("strict_events") != 100
        or prereg.get("validation_budget", {}).get("remaining_untouched_after_phase60")
        != 48900
    ):
        raise RuntimeError("Phase60 requires bounded fresh-validation replication")
    closeout = prereg["phase59_closeout_basis"]
    if prereg.get("gradient_execution_contract") != {
        ...
        raise RuntimeError("phase60 requires gradient-safe PID execution")
    if (
        prereg.get("study_id") != STUDY_ID
        or closeout.get("classification") != "feasibility_complete_no_promotion"
        or closeout.get("selected_next_factor") != "fresh_validation_replication"
        or closeout.get("sealed_test_accessed") is not False
    ):
        raise RuntimeError("phase60 closeout basis is invalid")
    return closeout
...
def validate_retained_basis(retained, retained_binding):
    if (
        ...
        or retained.get("version") != "phase59-retained-tree-export-v1"
        ...
        != "authenticated_refined_checkpoint_evaluation_recovery"
        or retained.get("sealed_test_accessed") is not False
    ):
        raise RuntimeError("phase60 retained-tree evidence is incomplete")
...
def validate_seed_contract(config: dict[str, Any], cohort_seed: int) -> None:
    ...
    if (
        ...
        or config["seed"] != 20260927
        or config["seed"] != cohort_seed
        or config.get("balanced_level_replay_contract", {}).get("seed")
        != config["seed"]
    ):
        raise RuntimeError("phase60 training, replay, and cohort seeds must agree")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase60 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase60 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase60 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase60 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase60 hashed input changed: {binding['path']}")
    ...
    if (
        ...
        or evidence_record.get("status") != "FEASIBILITY_COMPLETE"
    ):
        raise RuntimeError("phase60 closeout evidence changed")
    expected_arm = next(
    ...
    if (
        ...
        raise RuntimeError("phase60 preregistered checkpoint or config changed")
    ...
    if set(capacity) != set(ARM_ROLES) or any(
        ...
        raise RuntimeError("phase60 capacity admission is invalid")

    from scripts.run_phase60_pretraining import validate_pretraining_contract
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase60-cohort-v1"
        ...
        raise RuntimeError(
            "phase60 fresh-selection/fresh-strict cohort contract is invalid"
        )
    ...
    retained_binding = prereg["phase59_retained_metric_basis"]
    retained_path = _repo_file(retained_binding["path"])
    if sha256(retained_path) != retained_binding["sha256"]:
        raise RuntimeError("phase60 retained-tree evidence hash changed")
    ...
    if evaluation_contract.get("retained_tree_checks") != {
        ...
        raise RuntimeError("phase60 requires complete retained-tree and beam checks")
    ...
    if (
        ...
        raise RuntimeError("phase60 scientific selection/gate contract changed")
    if (
        ...
        raise RuntimeError("phase60 shared weighted/recovery contract changed")
    if config.get("max_cardinality_by_level") != [
        ...
        raise RuntimeError("phase60 repaired cardinality contract changed")
    ...
    if (
        ...
        raise RuntimeError("phase60 fresh corrected statistics are not bound")
    expected_freeze = {
    ...
    if (
        ...
        raise RuntimeError("phase60 corrected adaptation comparison changed")
    ...
    if (
        ...
        raise RuntimeError("phase60 corrected-source boundary is missing")
    ...
    if (
        ...
        raise RuntimeError("phase60 controlled pretraining-transfer contract changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (
        ROOT / "artifacts/runs/ht-reconstruction-phase60-20260925"
    ).resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase60 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase60 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase60 outputs")
    ...
    from scripts.run_phase60_pretraining import run_pretraining_refinement
    ...
    if result.steps != 4_376 or sha256(source) != source_before:
        raise RuntimeError("phase60 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase60 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 4_376:
        raise RuntimeError("phase60 final checkpoint is not step 4376")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase60 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase60-training-v1",

#!/usr/bin/env python3
"""Validate immutable Phase58 reports and export complete reconstruction metrics."""
...
ARMS = {
    "pretraining_balance_control": "16670555",
    "lower_late_pid_pretraining": "16670556",
}
BEAM_METRICS = (
    "source_recall",
    "source_precision",
...
def verify_receipt(receipt):
    ...
    if (
        ...
        != (
            "failed"
            if receipt["arm_role"] == "pretraining_balance_control"
            else "completed"
        )
        or receipt["exit_status"]
        != (1 if receipt["arm_role"] == "pretraining_balance_control" else 0)
        or receipt["sealed_test_accessed"]
    ):
        raise ValueError("Phase58 run did not complete within its authority")


def build(source, recovery):
    prereg = load(
        source / "configs/reconstruction/ht_reconstruction_phase58_20260923.json"
    )
    ...
    payload = {
        "audit_version": "phase58-closeout-v1",
        "status_date": "2026-09-24",
        "status": "INCOMPLETE_COMPARISON",
        "metric_completeness": "COMPLETE_AVAILABLE_ARTIFACTS",
    ...
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase58-20260923/{arm}/{job}"
        receipt_path = (
            source
            / f"artifacts/slurm/reconstruction-phase58/jobs/{job}/attempt-00/receipt.json"
        )
        ...
        for name, binding in receipt["artifacts"].items():
            if binding is not None:
                assert digest(source / binding["path"]) == binding["sha256"]
        if arm == "pretraining_balance_control":
            assert all(
                receipt["artifacts"][k] is None
                for k in (
                    "full_decay_gates",
            ...
            assert (
                not (run / "training").exists() and not (run / "result.json").exists()
            )
            payload["arms"][arm] = {
                "status": "FAILED_PRETRAINING",
                "reconstruction_status": "UNAVAILABLE_NOT_TRAINED",
                "endpoints": None,
                "gates": None,
                "selected": None,
                "all_gates_passed": None,
                "primary_repeat_identical": None,
                "completed_pretraining_steps": 1752,
                "failed_attempted_step": 1753,
                "failure_gradient_ratio": 23.1619,
                "gradient_ratio_limit": 20.0,
            }
            payload["source_hashes"].append(digest(receipt_path))
            continue
        result, gates = (
            load(run / "result.json"),
            load(recovery / arm / "full-decay-gates.json"),
        )
        assert (
        ...
        payload["source_hashes"].extend(
            [
                ...
                digest(recovery / arm / "full-decay-gates.json"),
            ]
        )
    return payload
...
def main():
    ...
    payload["source_hashes"].append(
        digest(args.details_dir / "training-history/manifest.json")
    )
    payload.update(
        training_history_scalar_rows=history["total_scalar_rows"],
        training_history_log_records=sum(
            a["log_records"] for a in history["arms"].values()
        ),
        training_history_checkpoint_records=sum(
            len(a["checkpoints"]) for a in history["arms"].values()
        ),
    )
    args.details_dir.mkdir(parents=True, exist_ok=True)
    (args.details_dir / "phase58-all-metrics.json").write_text(
        json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False)
        + "\n"
    )
    with gzip.open(
        args.details_dir / "phase58-all-metrics.csv.gz", "wt", newline=""
    ) as handle:
    ...
    payload["metric_rows"] = [
        ...
        if not row["metric"].startswith(
            ("summaries_by_source_category.", "summaries_by_target_shape.")
        )
    ]
    ...
    (ROOT / "docs/_ext/phase58_metric_registry.json").write_text(
        json.dumps(
            [[r["arm"], r["view"], r["metric"]] for r in payload["metric_rows"]],
            separators=(",", ":"),
        )
        + "\n"
    )
    print(
        json.dumps(
            {
                "aggregate_rows": len(payload["metric_rows"]),

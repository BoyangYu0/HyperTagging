#!/usr/bin/env python3
"""Freeze the admission-repaired phase-40r1 replacement campaign."""
...
PARENT = ROOT / "configs/reconstruction/ht_reconstruction_phase40_20260907.json"
...
PHASE40_STUDY_ID = "phase40-hierarchy-aware-reconstruction-20260907"
FAILURE_EVIDENCE = (
    (
        "doubled_data_control",
        ROOT / "artifacts/slurm/reconstruction-phase40/jobs/16361396/attempt-00/receipt.json",
        ROOT / "artifacts/slurm/phase40-doubled_data_control-20260907-16361396.err",
    ),
    (
        "doubled_data_query_scale",
        ROOT / "artifacts/slurm/reconstruction-phase40/jobs/16361397/attempt-00/receipt.json",
        ROOT / "artifacts/slurm/phase40-doubled_data_query_scale-20260907-16361397.err",
    ),
)
...
def main() -> int:
    ...
    if (
        cohort.get("study_id") != PHASE40_STUDY_ID
        or cohort.get("all_required_overlaps_zero") is not True
        or any(cohort.get("overlap_audit", {}).values())
    ):
        raise RuntimeError("phase40 untouched cohort is not reusable")

    failure_evidence = []
    for role, receipt_path, error_log_path in FAILURE_EVIDENCE:
        ...
        if (
            ...
            or "OverflowError: 1 mothers exceed cardinality capacity" not in error_log
        ):
            raise RuntimeError(f"phase40 failure evidence is not admission-only: {role}")
        failure_evidence.append(
            {
                ...
                "error_log": str(error_log_path.relative_to(ROOT)),
                "error_log_sha256": sha256(error_log_path),
                "terminal_stage": "trainer",
                "optimizer_steps": 0,
                "checkpoint_selection_cohort_evaluated": False,
                "sealed_test_accessed": False,
            }
        )

    common = deepcopy(parent["common_training_contract"])
    common["max_cardinality_by_level"] = [
        [level, 15 if level == 4 else value]
        for level, value in common["max_cardinality_by_level"]
    ]
    common.update(
        {
            "seed": 20260909,
    ...
    for arm in parent["arms"]:
        ...
        if (
            ...
            raise RuntimeError(f"phase40r1 capacity repair is insufficient: {role}")
    ...
    payload = {
        "preregistration_version": "hypertagging-reconstruction-phase40r1-preregistration-v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "study_id": STUDY_ID,
        "scientific_question": parent["scientific_question"],
        "parent_phase40": {"path": str(PARENT.relative_to(ROOT)), "sha256": sha256(PARENT)},
        "replacement_basis": {
            "classification": "admission_only_capacity_repair",
            "phase40_optimizer_steps": 0,
            "phase40_checkpoint_selection_cohort_evaluated": False,
            "phase40_sealed_test_accessed": False,
            "only_scientific_contract_change": "level_4_max_cardinality_14_to_15",

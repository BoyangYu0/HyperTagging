#!/usr/bin/env python3
"""Execute one fail-closed phase-57 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase57-contract-v1"
STUDY_ID = "phase57-pretraining-objective-balance-20260922"
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    from scripts.run_reconstruction_phase56 import validation_exclusions as previous_exclusions
    previous_path = _repo_file(cohort['source_bindings']['phase56_cohort']['path'])
    previous = json.loads(previous_path.read_text())
    if sha256(previous_path) != cohort['source_bindings']['phase56_cohort']['sha256']:
        raise RuntimeError('Phase57 previous cohort changed')
    allowed = previous['checkpoint_selection_event_uids']
    if cohort['checkpoint_selection_event_uids'] != allowed or cohort.get('permitted_selection_reuse_count') != 1000:
        raise RuntimeError('Phase57 permits only exact Phase56 selection reuse')
    historical = set(previous_exclusions(previous)) | set(previous['event_uids']) | set(allowed)
    strict = set(cohort['event_uids'])
    if len(strict) != 100 or strict & historical:
        raise RuntimeError('Phase57 strict cohort must be untouched')
    values = historical - set(allowed)
    identity = excluded_event_uids_contract(tuple(values))
    if identity['excluded_event_uid_count'] != cohort['validation_exclusion_event_uid_count'] or identity['excluded_event_uids_sha256'] != cohort['validation_exclusion_event_uids_sha256']:
        raise RuntimeError('Phase57 exclusion identity changed')
    return tuple(sorted(values))


def validate_closeout_basis(prereg: dict[str, Any]) -> dict[str, Any]:
    closeout = prereg["phase56_closeout_basis"]
    if prereg.get('gradient_execution_contract') != {'autocast_weight_cache': False, 'trainable_pid_supervision_requires_gradient': True, 'phase48_pid_adaptation_effect_was_not_executed': True}:
        raise RuntimeError('phase57 requires gradient-safe PID execution')
    if (prereg.get("study_id") != STUDY_ID
        or closeout.get("classification") != "incomplete_control_objective_preflight_failure_no_promotion"
        or closeout.get("selected_next_factor") != "late_pretraining_leaf_pid_phase_weights"
        or closeout.get("sealed_test_accessed") is not False):
        raise RuntimeError("phase57 closeout basis is invalid")
    return closeout
...
def validate_seed_contract(config: dict[str, Any], cohort_seed: int) -> None:
    ...
    if (type(config.get("seed")) is not int
        or config["seed"] != 20260924
        or config["seed"] != cohort_seed
        or config.get("balanced_level_replay_contract", {}).get("seed") != config["seed"]):
        raise RuntimeError("phase57 training, replay, and cohort seeds must agree")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("phase57 contract identity or authority is invalid")

    expected_sha = str(contract["expected_git_sha"])
    expected_tag = str(contract["expected_git_tag"])
    if _git("rev-parse", "HEAD") != expected_sha:
        raise RuntimeError("phase57 HEAD differs from the frozen contract")
    if _git("rev-list", "-n", "1", expected_tag) != expected_sha:
        raise RuntimeError("phase57 implementation tag does not bind HEAD")
    if _git("status", "--porcelain", "--untracked-files=no"):
        raise RuntimeError("phase57 tracked worktree is dirty")

    for binding in contract["hashed_inputs"]:
        source = _repo_file(str(binding["path"]))
        if sha256(source) != binding["sha256"]:
            raise RuntimeError(f"phase57 hashed input changed: {binding['path']}")
    ...
    if (sha256(evidence) != closeout["sha256"]
        ...
        raise RuntimeError("phase57 closeout evidence changed")
    ...
    if (contract["checkpoint"] != expected_arm["checkpoint"]
        ...
        raise RuntimeError("phase57 preregistered checkpoint or config changed")
    ...
    if set(capacity) != set(ARM_ROLES) or any(
        ...
        raise RuntimeError("phase57 capacity admission is invalid")

    from scripts.run_phase57_pretraining import validate_pretraining_contract
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase57-cohort-v1"
        ...
        raise RuntimeError("phase57 declared reused-selection/fresh-strict cohort contract is invalid")
    ...
    if evaluation_contract.get("retained_tree_checks") != {
        ...
        raise RuntimeError("phase57 requires complete retained-tree and beam checks")
    ...
    if (
        ...
        raise RuntimeError("phase57 scientific selection/gate contract changed")
    if (
        ...
        raise RuntimeError("phase57 shared weighted/recovery contract changed")
    if config.get("max_cardinality_by_level") != [
        [1, 9], [2, 16], [3, 15], [4, 15], [5, 13], [6, 2]
    ]:
        raise RuntimeError("phase57 repaired cardinality contract changed")
    ...
    if (sha256(statistics_path) != statistics_binding['sha256']
        ...
        raise RuntimeError('phase57 fresh corrected statistics are not bound')
    expected_freeze = {"pretraining_balance_control": 4376, "lower_late_pid_pretraining": 4376}[contract["arm_role"]]
    if (config.get("freeze_pretrained_encoder_steps") != 2188
        ...
        raise RuntimeError("phase57 corrected adaptation comparison changed")
    ...
    if (boundary.get("fresh_train_only_normalization_required") is not True
        ...
        raise RuntimeError("phase57 corrected-source boundary is missing")
    ...
    if (
        ...
        raise RuntimeError("phase57 controlled pretraining-transfer contract changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (ROOT / "artifacts/runs/ht-reconstruction-phase57-20260922").resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase57 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase57 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase57 outputs")
    ...
    from scripts.run_phase57_pretraining import run_pretraining_refinement
    ...
    if result.steps != 4_376 or sha256(source) != source_before:
        raise RuntimeError("phase57 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase57 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 4_376:
        raise RuntimeError("phase57 final checkpoint is not step 4376")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase57 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase57-training-v1",

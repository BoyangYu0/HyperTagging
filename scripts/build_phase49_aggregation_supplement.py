...
def build(source):
    payload={'version':'phase49-aggregation-supplement-v1','status':'COMPLETE','source_hashes':[],'metric_rows':[]}
    for arm,job in ARMS.items():
        for path in sorted((source/f'artifacts/runs/ht-reconstruction-phase49-20260914/{arm}/{job}/full-decay-reports').glob('*.json')):
...
def main():
    ...
    (ROOT/'docs/_ext/phase49_aggregation_metric_registry.json').write_text(json.dumps(registry,separators=(',',':'))+'\n');print('PASS',len(registry))

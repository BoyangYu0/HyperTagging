#!/usr/bin/env python3
"""Execute one fail-closed phase-42 reconstruction training contract."""
...
CONTRACT_VERSION = "hypertagging-reconstruction-phase42-contract-v1"
STUDY_ID = "phase42-pretraining-transfer-20260909"
COHORT_STUDY_ID = STUDY_ID
ARM_ROLES = (
    "pretrain81096_control",
    "pretrain108128",
)
...
def validation_exclusions(cohort: dict[str, Any]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("phase42 validation exclusion identity changed")
...
def verify_contract(path: Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (sha256(evidence) != closeout["sha256"]
        ...
        raise RuntimeError("phase42 closeout evidence changed")
    ...
    if (contract["checkpoint"] != expected_arm["checkpoint"]
        ...
        raise RuntimeError("phase42 preregistered checkpoint or config changed")
    ...
    if set(capacity) != set(ARM_ROLES) or any(
        ...
        raise RuntimeError("phase42 capacity admission is invalid")
    ...
    if (
        ...
        != "hypertagging-reconstruction-phase42-cohort-v1"
        ...
        raise RuntimeError("phase42 untouched cohort contract is invalid")
    ...
    if (
        ...
        raise RuntimeError("phase42 scientific selection/gate contract changed")
    if (
        ...
        raise RuntimeError("phase42 shared weighted/recovery contract changed")
    if config.get("max_cardinality_by_level") != [
        [1, 9], [2, 16], [3, 15], [4, 15], [5, 13], [6, 2]
    ]:
        raise RuntimeError("phase42 repaired cardinality contract changed")
    ...
    if (
        ...
        raise RuntimeError("phase42 controlled pretraining-transfer contract changed")

    output_root = (ROOT / contract["output_root"]).resolve()
    expected_parent = (ROOT / "artifacts/runs/ht-reconstruction-phase42-20260909").resolve()
    if expected_parent not in output_root.parents:
        raise RuntimeError("phase42 output root escaped its namespace")
    runtime = {
...
def main() -> int:
    ...
    if output_root not in output.parents or output_root not in training_output.parents:
        raise RuntimeError("phase42 outputs escape the arm output root")
    if output.exists() or training_output.exists():
        raise RuntimeError("refusing to reuse phase42 outputs")
    ...
    if result.steps != 4_376 or sha256(source) != source_before:
        raise RuntimeError("phase42 budget or immutable source contract failed")
    if result.transfer_report is None or result.transfer_report.coverage < 1.0:
        raise RuntimeError("phase42 encoder transfer coverage is incomplete")
    ...
    if checkpoints["final"]["step"] != 4_376:
        raise RuntimeError("phase42 final checkpoint is not step 4376")
    if any(
        audit["step"] not in ALLOWED_SELECTION_STEPS
        for name, audit in checkpoints.items()
        if name != "final"
    ):
        raise RuntimeError("phase42 track selected an unregistered step")
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase42-training-v1",

#!/usr/bin/env python3
"""Verify additive Phase56 tree coverage against preserved native reports."""
import argparse
import csv
import gzip
import hashlib
...
ARMS = ('stronger_parent_pretraining',)
...
def build(retained):
    output = {'version': 'phase56-retained-tree-export-v1', 'status': 'COMPLETE',
              'unavailable_arms': {'late_adaptation_control': 'UNAVAILABLE_NOT_TRAINED'}, 'source_boundary': 'native_phase56_one_arm_pretraining_failure',
              'evaluator_revision': '8b2a994b0ef5a7dfa4011a1eccad5c966a4fcd93', 'sealed_test_accessed': False,
...
def main():
    ...
    with gzip.open(args.details_dir / 'phase56-retained-all-metrics.csv.gz', 'wt', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=['arm', 'view', 'metric', 'value'])
        writer.writeheader()
        writer.writerows(payload['metric_rows'])
    tree_export = args.details_dir / 'phase56-retained-tree-metrics.csv.gz'
    tree_count = 0
    ...
    (args.details_dir / 'phase56-retained-all-metrics.json').write_text(json.dumps(payload, sort_keys=True, separators=(',', ':'), allow_nan=False) + '\n')
    ...
    (ROOT / 'docs/_ext/phase56_retained_metric_registry.json').write_text(json.dumps(registry, separators=(',', ':')) + '\n')

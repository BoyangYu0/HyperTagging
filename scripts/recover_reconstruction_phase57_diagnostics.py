#!/usr/bin/env python3
"""Recover Phase57 in-sample diagnostics; never certify independent gate passage."""
...
def _metric(report: dict[str, Any], *keys: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"phase57 metric is malformed: {'.'.join(keys)}")
    return {
...
def refined_evaluation_runtime(training_result, runtime):
    ...
    if (refinement.get("status") != "COMPLETED" or refinement.get("step") != 2188
        ...
        raise RuntimeError("Phase57 refined evaluation checkpoint lineage is invalid")
...
def verify_evaluation_contract(path, source_root=None):
    ...
    command = [sys.executable, "-c",
        "import json,pathlib,sys; from scripts.run_reconstruction_phase57 import verify_contract; "
...
def validate_diagnostic_evidence(record, contract):
    if (record.get('status') != 'TRAINED_COHORT_DEVIATION_DIAGNOSTIC_ONLY'
        or record.get('strict_selection_overlap') != 100
        or record.get('independent_validation') is not False
        or record.get('original_receipt_status') != 'failed'
        or record.get('contract_sha256') != contract['contract_sha256']):
        raise RuntimeError('Phase57 requires authenticated selection-contaminated diagnostic evidence')
...
def main() -> int:
    ...
    validate_diagnostic_evidence(training_result, contract)
    runtime = refined_evaluation_runtime(training_result, runtime)
    output_dir = args.output_dir.resolve()
    output = args.output.resolve()
    if output.exists() or output_dir.exists():
        raise RuntimeError("refusing to reuse phase57 full-decay outputs")
    ...
    for label, filename in TRACKS.items():
        ...
        if step not in ALLOWED_SELECTION_STEPS:
            raise RuntimeError(f"phase57 {label} selected invalid step {step}")
    ...
    if (
        ...
        raise RuntimeError("phase57 beam-search validation contract failed")
    artifacts["primary_complete_target"].update(
        {
    ...
    payload = {
        "result_version": "hypertagging-reconstruction-phase57-full-decay-gates-v1",
        "status": "DIAGNOSTIC_COMPLETE",
        "scientific_classification": "ALL_STRICT_EVENTS_USED_FOR_CHECKPOINT_SELECTION",
        "independent_validation": False,
        "strict_selection_overlap": 100,
        ...
        "all_numerical_thresholds_passed_diagnostic_only": all_passed,
        "all_preregistered_gates_passed": False,
        "eligible_for_cross_arm_gate_review": False,
        "longer_run_authorized": False,
        "promotion_authorized": False,
        "sealed_test_request_authorized": False,
        "authority_note": (
            "All strict events were used for selection. These are in-sample diagnostics, "
            "not independent validation or promotion evidence. Original failed receipts are preserved."
        ),
    }

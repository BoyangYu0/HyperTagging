#!/usr/bin/env python3
"""Verify additive Phase48 tree coverage against preserved native reports."""
import argparse
import csv
import gzip
import hashlib
...
def report_paths(source, arm):
    job = {'late_adaptation_control': '16491438', 'late_pid_adaptation': '16491439'}[arm]
    return sorted((source / f'artifacts/runs/ht-reconstruction-phase48-20260914/{arm}/{job}/full-decay-reports').glob('*.json'))
...
def build(retained):
    output = {'version': 'phase48-retained-tree-export-v1', 'status': 'COMPLETE',
              'original_job_status': 'COMPLETED', 'source_boundary': 'corrected_scientific_audit_source',
              'evaluator_revision': '1d3424ed5a0dc12e8ec10039a951919710131a90', 'sealed_test_accessed': False,
...
def main():
    ...
    with gzip.open(args.details_dir / 'phase48-retained-all-metrics.csv.gz', 'wt', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=['arm', 'view', 'metric', 'value'])
        writer.writeheader()
        writer.writerows(payload['metric_rows'])
    tree_export = args.details_dir / 'phase48-retained-tree-metrics.csv.gz'
    tree_count = 0
    ...
    (args.details_dir / 'phase48-retained-all-metrics.json').write_text(json.dumps(payload, sort_keys=True, separators=(',', ':'), allow_nan=False) + '\n')
    ...
    (ROOT / 'docs/_ext/phase48_retained_metric_registry.json').write_text(json.dumps(registry, separators=(',', ':')) + '\n')

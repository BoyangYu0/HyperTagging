#!/usr/bin/env python3
"""Export all recorded Phase51 checkpoint and training-log scalar metrics."""
...
from scripts.build_reconstruction_phase51_closeout import ARMS, verify_receipt  # noqa: E402
...
def main():
    ...
    with gzip.open(out/'phase51-training-history.csv.gz', 'wt', newline='') as csv_file, gzip.open(out/'phase51-training-history.jsonl.gz', 'wt') as json_file:
        ...
        for arm, job in ARMS.items():
            receipt = source/f'artifacts/slurm/reconstruction-phase51/jobs/{job}/attempt-00/receipt.json'
            verify_receipt(json.loads(receipt.read_text()))
            training = source/f'artifacts/runs/ht-reconstruction-phase51-20260916/{arm}/{job}/training'

#!/usr/bin/env python3
"""Validate immutable Phase55 reports and export complete reconstruction metrics."""
...
ARMS = {"late_adaptation_control": "16597494", "stronger_parent_pretraining": "16597495"}
...
def verify_receipt(receipt):
    ...
    if receipt["status"] != "failed" or receipt["exit_status"] != 1 or receipt["sealed_test_accessed"]:
        raise ValueError("Phase55 run did not complete within its authority")


def build(source, recovery):
    prereg = load(source / "configs/reconstruction/ht_reconstruction_phase55_20260920.json")
    ...
    payload = {"audit_version": "phase55-closeout-v1", "status_date": "2026-09-21",
               ...
               "next_study_status": "PREPARED", "source_boundary": "authenticated_refined_checkpoint_evaluation_recovery", "original_job_status": "FAILED_POST_TRAINING_EVALUATION", "arms": {}, "source_hashes": [], "metric_rows": []}
    for arm, job in ARMS.items():
        run = source / f"artifacts/runs/ht-reconstruction-phase55-20260920/{arm}/{job}"
        receipt_path = source / f"artifacts/slurm/reconstruction-phase55/jobs/{job}/attempt-00/receipt.json"
...
def main():
    ...
    (args.details_dir / "phase55-all-metrics.json").write_text(json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n")
    with gzip.open(args.details_dir / "phase55-all-metrics.csv.gz", "wt", newline="") as handle:

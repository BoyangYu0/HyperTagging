...
import pytest
from scripts.run_reconstruction_phase50 import ARM_ROLES, validate_closeout_basis, validate_seed_contract, validation_exclusions
...
def test_phase50_changes_only_pid_head_freeze_between_arms():
    p = load('ht_reconstruction_phase50_20260915.json')
    ...
    previous = load('ht_reconstruction_phase49_20260914.json')
...
def test_phase50_cohort_and_evidence_are_bound():
    p = load('ht_reconstruction_phase50_20260915.json')
    c = load('ht_reconstruction_phase50_validation_cohort_20260915.json')
    ...
    for key in ('phase49_closeout_basis','phase49_retained_metric_basis','untouched_validation_cohort'):
        record = p[key]
...
def test_phase50_rejects_stale_seeds(change):
    config = {'seed':20260917,'balanced_level_replay_contract':{'seed':20260917}}
    cohort_seed = 20260917
    if change == 'training': config['seed'] = 20260915
    if change == 'replay': config['balanced_level_replay_contract']['seed'] = 20260915
    if change == 'cohort': cohort_seed = 20260915
...
def test_phase50_rejects_missing_gradient_execution_contract():
    p = load('ht_reconstruction_phase50_20260915.json')

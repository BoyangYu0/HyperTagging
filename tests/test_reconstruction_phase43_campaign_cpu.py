...
import pytest
from scripts.run_reconstruction_phase43 import ARM_ROLES, validation_exclusions
...
def test_phase43_changes_only_encoder_freeze_at_fixed_steps_and_data():
    p = load('ht_reconstruction_phase43_20260910.json')
    ...
    previous = load('ht_reconstruction_phase42_20260909.json')
...
def test_phase43_cohort_excludes_all_previous_tuning_and_evaluation():
    c = load('ht_reconstruction_phase43_validation_cohort_20260910.json')
    ...
    old = load('ht_reconstruction_phase42_validation_cohort_20260909.json')
    assert set(old['checkpoint_selection_event_uids']) | set(old['event_uids']) <= excluded


def test_phase43_rejects_tampered_exclusion_identity():
    c = load('ht_reconstruction_phase43_validation_cohort_20260910.json')
...
def test_phase43_preregistration_binds_review_and_cohort():
    p = load('ht_reconstruction_phase43_20260910.json')
    for record in (p['phase42_closeout_basis'], p['untouched_validation_cohort']):

...
import pytest
from scripts.run_reconstruction_phase47 import ARM_ROLES, validate_closeout_basis, validate_seed_contract, validation_exclusions
...
def test_phase47_changes_only_encoder_freeze_between_arms():
    p = load('ht_reconstruction_phase47_20260913.json')
    ...
    previous = load('ht_reconstruction_phase46_20260912.json')
...
def test_phase47_cohort_and_evidence_are_bound():
    p = load('ht_reconstruction_phase47_20260913.json')
    c = load('ht_reconstruction_phase47_validation_cohort_20260913.json')
    ...
    for key in ('phase46_closeout_basis','phase46_retained_metric_basis','untouched_validation_cohort'):
        record = p[key]
...
def test_phase47_rejects_stale_seeds(change):
    config = {'seed':20260914,'balanced_level_replay_contract':{'seed':20260914}}
    cohort_seed = 20260914
    if change == 'training': config['seed'] = 20260913
    if change == 'replay': config['balanced_level_replay_contract']['seed'] = 20260913
    if change == 'cohort': cohort_seed = 20260913

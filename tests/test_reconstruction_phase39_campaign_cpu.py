...
from scripts.run_reconstruction_phase39 import ARM_ROLES, CHECKPOINT_TRACKS


ROOT = Path(__file__).resolve().parents[1]
PREREGISTRATION = (
    ROOT / "configs/reconstruction/ht_reconstruction_phase39_20260907.json"
)
COHORT = (
    ROOT
...
def test_phase39_cohort_is_untouched_and_sealed_test_free() -> None:
...
def test_phase39_preregisters_single_factor_arms_and_hard_gates() -> None:
    ...
    assert reference["pointer_positive_weight"] == 24.0
    ...
    assert prereg["evaluation_contract"] == {
        "cpu_threads": 1,
...
def test_phase39_training_config_preserves_fractional_level_weights(
    tmp_path: Path,
) -> None:
    prereg = json.loads(PREREGISTRATION.read_text(encoding="utf-8"))
    candidate = {
...
def test_phase39_runner_requires_all_independent_tracks() -> None:
...
def test_phase39_full_decay_binds_manifest_and_preregistered_thresholds() -> None:
    source = (ROOT / "scripts/run_reconstruction_phase39_full_decay.py").read_text(
        encoding="utf-8"
    )

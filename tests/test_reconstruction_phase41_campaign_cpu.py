"""CPU contract tests for the preregistered phase-41 reconstruction campaign."""
...
from scripts.run_reconstruction_phase41 import ARM_ROLES, CHECKPOINT_TRACKS


ROOT = Path(__file__).resolve().parents[1]
PREREGISTRATION = (
    ROOT / "configs/reconstruction/ht_reconstruction_phase41_20260909.json"
)
COHORT = (
    ROOT
    / "configs/reconstruction/ht_reconstruction_phase41_validation_cohort_20260909.json"
)


def test_phase41_data_is_exactly_doubled_and_test_is_sealed() -> None:
    manifest = json.loads(
        (
            ROOT
...
def test_phase41_cohort_is_untouched_and_sealed_test_free() -> None:
    ...
    assert cohort["source_bindings"]["phase40_cohort"][
        "checkpoint_selection_event_uid_count"
    ] == 2_000
    assert cohort["source_bindings"]["phase40_cohort"][
        "evaluation_event_uid_count"
    ] == 100
    assert cohort["source_bindings"]["phase41_training_role"][
        "event_uid_count"
    ] == 70_000


def test_phase41_preregisters_single_factor_arms_and_hard_gates() -> None:
    ...
    assert common["max_cardinality_by_level"] == [
        [1, 9], [2, 16], [3, 15], [4, 15], [5, 13], [6, 2]
    ]
    closeout = prereg["phase40r1_closeout_basis"]
    assert closeout["classification"] == "completed_controlled_study_no_promotion"
    assert closeout["selected_next_factor"] == (
        "level_1_pointer_positive_weight_32_to_24"
    )
    assert closeout["query_scale_continuation_authorized"] is False
    assert len(closeout["evidence"]) == 2
    for report in prereg["capacity_admission"]["reports_by_arm"].values():
        assert report["production_training_allowed"] is True
        assert report["query_overflow_count"] == 0
        assert report["cardinality_overflow_count"] == 0
    reference = prereg["arms"][0]["overrides"]
    candidate = prereg["arms"][1]["overrides"]
    assert common["rollout_pointer_threshold"] == 0.35
    assert common["rollout_object_threshold"] == 0.6
    ...
    assert common["pointer_positive_weights_by_level"] == []
    ...
    assert candidate["pointer_positive_weights_by_level"] == [[1, 24.0]]
...
def test_phase41_training_config_preserves_fractional_level_weights(
    tmp_path: Path,
) -> None:
    prereg = json.loads(PREREGISTRATION.read_text(encoding="utf-8"))
    candidate = {
    ...
    assert config.pointer_positive_weights_by_level == ((1, 24.0),)
...
def test_phase41_runner_requires_all_independent_tracks() -> None:
...
def test_phase41_full_decay_binds_manifest_and_preregistered_thresholds() -> None:
    source = (ROOT / "scripts/run_reconstruction_phase41_full_decay.py").read_text(
        encoding="utf-8"
    )

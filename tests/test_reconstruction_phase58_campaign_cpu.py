...
import pytest
from scripts.run_reconstruction_phase58 import (
...
from scripts.run_phase58_pretraining import (
...
def prereg():
    return json.loads(
        (
            ROOT / "configs/reconstruction/ht_reconstruction_phase58_20260923.json"
        ).read_text()
    )
...
def test_real_selector_exactly_matches_preregistered_sequence_and_excludes_every_other_uid():
    ...
    r = preflight_selection(
        c,
        {"max_validation_events": 1000, "seed": 20260925, "scientific_mode": True},
        ex,
    )
    assert r == {
        "status": "PASS",
    ...
    assert (
        set(c["event_uids"]) <= set(ex) and c["remaining_untouched_after_phase58"] == 25
    )
...
def test_bad_cohort_or_actual_selector_parameters_fail(change):
    c = cohort()
    cfg = {"seed": 20260925, "max_validation_events": 1000, "scientific_mode": True}
    ex = validation_exclusions(c)
    if change == "mode":
...
def test_phase58_factor_isolation_and_bound_fresh_strict_cohort(tmp_path):
    p = prereg()
    c = cohort()
    ...
    assert configs[0].pop("leaf_pid_phase_weights") == [1, 1, 0.4, 0.4]
    assert configs[1].pop("leaf_pid_phase_weights") == [1, 1, 0.2, 0.2]
...
def test_phase58_rejects_guard_and_factor_drift(key, value):
    p = prereg()

...
import pytest
from scripts.run_reconstruction_phase56 import ARM_ROLES, validate_closeout_basis, validate_seed_contract, validation_exclusions
...
def test_phase56_changes_only_pretraining_parent_ranking_weight():
    p = load('ht_reconstruction_phase56_20260921.json')
    ...
    assert a['max_validation_events'] == a['rollout_validation_events'] == 1000
    assert p['pretraining_refinement']['training_presentations'] == 70000
    assert p['validation_budget']['remaining_untouched_after_phase56'] == 391
    ...
    previous = load('ht_reconstruction_phase55_20260920.json')
...
def test_phase56_cohort_and_evidence_are_bound():
    p = load('ht_reconstruction_phase56_20260921.json')
    c = load('ht_reconstruction_phase56_validation_cohort_20260921.json')
    excluded = set(validation_exclusions(c))
    selected,evaluated = set(c['checkpoint_selection_event_uids']),set(c['event_uids'])
    assert len(selected) == 1000 and len(evaluated) == 100
    ...
    for key in ('phase55_closeout_basis','phase55_retained_metric_basis','untouched_validation_cohort'):
        record = p[key]
...
def test_phase56_rejects_stale_seeds(change):
    config = {'seed':20260923,'balanced_level_replay_contract':{'seed':20260923}}
...
def test_phase56_rejects_missing_gradient_execution_contract():
    p = load('ht_reconstruction_phase56_20260921.json')
...
def test_phase56_pretraining_contract_rejects_budget_and_factor_drift():
...
def test_phase56_pretraining_validation_never_uses_strict_cohort(tmp_path):
    from scripts.run_phase56_pretraining import refinement_config
    p = load('ht_reconstruction_phase56_20260921.json')
    cohort = load('ht_reconstruction_phase56_validation_cohort_20260921.json')

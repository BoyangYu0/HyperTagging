...
import pytest
from scripts.run_reconstruction_phase48 import ARM_ROLES, validate_closeout_basis, validate_seed_contract, validation_exclusions
...
def test_phase48_changes_only_pid_head_freeze_between_arms():
    p = load('ht_reconstruction_phase48_20260914.json')
    ...
    previous = load('ht_reconstruction_phase47_20260913.json')
...
def test_phase48_cohort_and_evidence_are_bound():
    p = load('ht_reconstruction_phase48_20260914.json')
    c = load('ht_reconstruction_phase48_validation_cohort_20260914.json')
    ...
    for key in ('phase47_closeout_basis','phase47_retained_metric_basis','untouched_validation_cohort'):
        record = p[key]
...
def test_phase48_rejects_stale_seeds(change):
    config = {'seed':20260915,'balanced_level_replay_contract':{'seed':20260915}}

...
import pytest
from scripts.run_reconstruction_phase49 import ARM_ROLES, validate_closeout_basis, validate_seed_contract, validation_exclusions
...
def test_phase49_changes_only_pid_head_freeze_between_arms():
    p = load('ht_reconstruction_phase49_20260914.json')
    ...
    previous = load('ht_reconstruction_phase48_20260914.json')
...
def test_phase49_cohort_and_evidence_are_bound():
    p = load('ht_reconstruction_phase49_20260914.json')
    c = load('ht_reconstruction_phase49_validation_cohort_20260914.json')
    ...
    for key in ('phase48_closeout_basis','phase48_retained_metric_basis','untouched_validation_cohort'):
        record = p[key]
...
def test_phase49_rejects_stale_seeds(change):
    config = {'seed':20260916,'balanced_level_replay_contract':{'seed':20260916}}
    cohort_seed = 20260916
    if change == 'training': config['seed'] = 20260914
    if change == 'replay': config['balanced_level_replay_contract']['seed'] = 20260914
    if change == 'cohort': cohort_seed = 20260914
...
def test_phase49_rejects_missing_gradient_execution_contract():
    p = load('ht_reconstruction_phase49_20260914.json')

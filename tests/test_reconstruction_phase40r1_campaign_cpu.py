"""CPU contract tests for the preregistered phase-40r1 reconstruction campaign."""
...
from scripts.run_reconstruction_phase40r1 import ARM_ROLES, CHECKPOINT_TRACKS


ROOT = Path(__file__).resolve().parents[1]
PREREGISTRATION = (
    ROOT / "configs/reconstruction/ht_reconstruction_phase40r1_20260908.json"
)
COHORT = (
    ROOT
...
def test_phase40r1_data_is_exactly_doubled_and_test_is_sealed() -> None:
    manifest = json.loads(
        (
            ROOT
...
def test_phase40r1_cohort_is_untouched_and_sealed_test_free() -> None:
...
def test_phase40r1_preregisters_single_factor_arms_and_hard_gates() -> None:
    prereg = json.loads(PREREGISTRATION.read_text(encoding="utf-8"))
    parent = json.loads(
        (ROOT / "configs/reconstruction/ht_reconstruction_phase40_20260907.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    expected_common = dict(parent["common_training_contract"])
    expected_common["max_cardinality_by_level"] = [
        [1, 9], [2, 16], [3, 15], [4, 15], [5, 13], [6, 2]
    ]
    assert common == expected_common
    assert prereg["arms"] == parent["arms"]
    assert common["max_cardinality_by_level"] == [
        [1, 9], [2, 16], [3, 15], [4, 15], [5, 13], [6, 2]
    ]
    assert prereg["replacement_basis"]["phase40_optimizer_steps"] == 0
    assert prereg["replacement_basis"][
        "phase40_checkpoint_selection_cohort_evaluated"
    ] is False
    assert len(prereg["replacement_basis"]["failure_evidence"]) == 2
...
def test_phase40r1_training_config_preserves_fractional_level_weights(
    tmp_path: Path,
) -> None:
    prereg = json.loads(PREREGISTRATION.read_text(encoding="utf-8"))
    candidate = {
...
def test_phase40r1_runner_requires_all_independent_tracks() -> None:
...
def test_phase40r1_full_decay_binds_manifest_and_preregistered_thresholds() -> None:
    source = (ROOT / "scripts/run_reconstruction_phase40r1_full_decay.py").read_text(
        encoding="utf-8"
    )

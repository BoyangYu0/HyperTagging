...
import pytest
from scripts.run_reconstruction_phase54 import ARM_ROLES, validate_closeout_basis, validate_seed_contract, validation_exclusions
...
def test_phase54_changes_only_recovery_objective_weight():
    p = load('ht_reconstruction_phase54_20260918.json')
    ...
    assert b.pop('recovery_objective_weight') == 1.0
    ...
    previous = load('ht_reconstruction_phase53_20260917.json')
...
def test_phase54_cohort_and_evidence_are_bound():
    p = load('ht_reconstruction_phase54_20260918.json')
    c = load('ht_reconstruction_phase54_validation_cohort_20260918.json')
    ...
    for key in ('phase53_closeout_basis','phase53_retained_metric_basis','untouched_validation_cohort'):
        record = p[key]
...
def test_phase54_rejects_stale_seeds(change):
    config = {'seed':20260921,'balanced_level_replay_contract':{'seed':20260921}}
    cohort_seed = 20260921
...
def test_phase54_rejects_missing_gradient_execution_contract():
    p = load('ht_reconstruction_phase54_20260918.json')

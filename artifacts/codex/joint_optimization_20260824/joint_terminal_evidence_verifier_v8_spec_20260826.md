# Joint terminal-evidence verifier v8 specification

Status: **ADMINISTRATIVELY CORRECTED SPECIFICATION PENDING INDEPENDENT REVIEW — AUTHORITY COMPILED, DOWNSTREAM ARTIFACTS REQUIRED**
...
The independent audit returned `BLOCK` because v6 retained the legacy SHA-index/Holm-rank-CI oracle instead of the effective-v7 PCG64 authority. The relayed review text has UTF-8 SHA-256 `d7ed7f475a71a68dcb27499f4829f43ba4299a4db7ce87d2b525d54b90dff287`. The subsequent administrative correction requires v8 to be the sole future implementation target; its relayed instruction has UTF-8 SHA-256 `b7a747fb42182c433abcaa6987ca2d43d4b9c350812d2989009da82eab791254`.

Every v7 scientific, oracle, schema, test-oracle, state-machine, downstream-gate, and canonicalization value is preserved. V8 changes only the exact administrative pointers enumerated in JSON: metadata, authority gap/rule closure text, allowed-change and preservation declarations, the v8 correction record, and implementation acceptance. The corrected PCG64 oracle, all 22 schemas, and all T001–T142 test definitions remain byte-identical as parsed JSON values.
...
All five constants are present in this specification, but implementation authority closes only through an independently reviewed eventual v8 transition binding the exact sealed v8 commit, tag, JSON SHA-256, and authority-artifact hashes. Blocked v6/v7 lineage closes no implementation authority. Nulls, mismatches, runtime selection, environment or CLI substitution, fixture overrides, alternate/latest authority, and bundle-supplied constants remain forbidden.
...
The future frozen no-sync implementation receipt must treat the independently reviewed v8 seal as its sole specification target. It must bind the exact commit obtained from annotated tag `ht-joint-terminal-evidence-verifier-v8-spec-20260826-v1^{}`, that fixed tag, the SHA-256 of the exact tagged v8 JSON bytes, implementation and test hashes, and lock/runtime hashes. V5, v6, and v7 verifier specs may be retained only as lineage references and may never be implementation targets or alternates.

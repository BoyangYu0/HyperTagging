...
def generate_status(repo_root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    if output.exists() and any(path.name not in {"index.rst", "status.json", "phase44-retained-metrics.json"} for path in output.iterdir()):

...
def generate_status(repo_root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    if output.exists() and any(path.name not in {"index.rst", "status.json", "phase44-retained-metrics.json", "phase45-metrics.json", "phase45-retained-metrics.json", "phase46-metrics.json", "phase46-retained-metrics.json"} for path in output.iterdir()):

...
| uv environment workflow | `/home/b/Boyang.Yu/.local/bin/uv`; `scripts/check_uv_lock_direct_dependencies.py`; `scripts/activate_env.sh`; `htenv`/`htgpu` | Verified | The corrected `uv.lock` SHA-256 is `38e6093...`; editable-root metadata contains all 10 direct runtime dependencies, including SciPy and PyYAML. Two consecutive `uv sync --frozen --all-extras` runs audited 124 packages without changes, the drift checker and `uv pip check` passed, and imports reported SciPy 1.18.0/PyYAML 6.0.3. Strict hashed GPU sync and the GPU lock-only preflight remain independently verified. Both shortcuts were verified in a fresh Bash shell. |
